...
RUNS = [
    (r, k, ARTIFACTS / r / ("handcrafted" if k == "handcrafted" else ""))
    for r in ("standard", "ood")
    for k in ("tfidf", "handcrafted")
]
...
class SavedExperimentTests(unittest.TestCase):
    ...
    def test_independent_scalar_routing_matches_every_frozen_grid_decision(self):
        for _, _, path in RUNS:
            ...
            probabilities = pd.read_parquet(path / "test_predictions.parquet").set_index(
                "prompt_id"
            )
            decisions = pd.read_parquet(path / "test_routing_decisions.parquet")
            for policy in table.itertuples():
                actual = (
                    decisions[decisions.policy_id == policy.policy_id]
                    .set_index("prompt_id")
                    .loc[probabilities.index, "selected_model_id"]
                )
                expected = []
                for row in probabilities.itertuples(index=False, name=None):
                    ...
                    if policy.kind == "static":
                        ...
                        choice = min(
                            MODEL_IDS,
                            key=lambda m: (
                                -(p[m] - policy.value * costs[m] / max_cost),
                                costs[m],
                                m,
                            ),
                        )
    ...
    def test_saved_selection_is_validation_feasible_minimum_cost(self):
        for regime, _, path in RUNS:
            ...
            np.testing.assert_allclose(
                [frozen["mean_training_cost_usd"][m] for m in MODEL_IDS], mean_c
            )
            table = pd.read_parquet(path / "validation_policy_grid.parquet")
            feasible = table[table.quality >= val_y[best].mean() - 0.005]
            winner = min(
                feasible.itertuples(), key=lambda r: (r.mean_cost_usd, -r.quality, r.policy_id)
            )
        ...
        for regime, _, path in RUNS:
            for split in ("validation", "test"):
                ...
                np.testing.assert_array_equal(
                    evaluation.success, y.to_numpy(dtype=float)[rows, columns]
                )
                np.testing.assert_array_equal(
                    evaluation.cost_usd, costs.to_numpy(dtype=float)[rows, columns]
                )
                self.assertFalse(
                    {"success", "cost_usd"}
                    & set(pd.read_parquet(path / f"{split}_routing_decisions.parquet").columns)
                )
    ...
    def test_baselines_match_frozen_preprocessing_references(self):
        ...
        for regime, _, path in RUNS:
            ...
            for ours, original in [
                ("best_single", "best_single"),
                ("always_cheapest", "always_cheapest"),
                ("oracle", "oracle_cheapest_success_with_fallback"),
            ]:
                a = evaluation[evaluation.policy_id == ours].sort_values("prompt_id")
                b = reference[
                    (reference.regime == regime) & (reference.policy == original)
        ...
        for regime, _, path in RUNS:
            ...
            for pid in ("deployable_router", "best_single", "always_cheapest", "oracle"):
                ...
                self.assertAlmostEqual(
                    e.groupby("dataset").success.mean().mean(), result["macro"][pid]["quality"]
                )
            ...
            for name, ci in result["bootstrap"]["ci_95"].items():
                np.testing.assert_allclose(
                    np.quantile(samples[name], [0.025, 0.975]), ci, atol=1e-15
                )
            ...
            strata = [
                a[a.dataset == d].sort_values("leakage_group").index.to_numpy()
            ...
            for _ in range(25):
                idx = np.concatenate(
                    [indices[rng.integers(len(indices), size=len(indices))] for indices in strata]
                )
                first.append(
                    [
                        float((a.success.to_numpy()[idx] - b.success.to_numpy()[idx]).mean()),
                        float(
                            1
                            - a.cost_usd.to_numpy()[idx].mean() / b.cost_usd.to_numpy()[idx].mean()
                        ),
                    ]
                )
            np.testing.assert_allclose(
                samples[["quality_delta", "cost_savings"]].iloc[:25], first, atol=1e-15
            )
    ...
    def test_cv_folds_and_ood_fits_are_independent_and_train_only(self):
        ...
        self.assertNotEqual(
            standard.metadata["training_prompt_ids_hash"], ood.metadata["training_prompt_ids_hash"]
        )
