...
from experiments.fixed_reference_router import canonical_part, run_regime, source_evidence
...
from routing_ml.crossfit import CrossfitPart
...
def synthetic_part(name="train", regime="standard"):
    ...
    p = pd.DataFrame(
        dict(
            prompt_id=[f"{name}_{i}" for i in range(n)],
            prompt=[
                (
                    "mathquadratic algebra equation "
                    if i < n // 2
                    else "historyempire factual answer "
                )
                + str(i)
                for i in range(n)
            ],
            dataset=["aime"] * (n // 2) + ["simpleqa"] * (n // 2),
            leakage_group=[f"{name}_group_{i // 2}" for i in range(n)],
        )
    ).set_index("prompt_id")
    ...
    return CrossfitPart(
        ...
        name,
        p,
        pd.DataFrame(y, index=p.index, columns=MODEL_IDS),
        pd.DataFrame(c, index=p.index, columns=MODEL_IDS),
    )
...
class FixedReferenceUnitTests(unittest.TestCase):
    def test_reference_is_gpt5_even_if_another_model_has_higher_validation_quality(self):
        p = pd.DataFrame(np.full((2, 8), 0.1), columns=MODEL_IDS)
        p[MODEL_IDS[REFERENCE_INDEX]] = [0.8, 0.8]
        p[MODEL_IDS[0]] = [0.9, 0.79]
        costs = np.arange(1, 9)
    ...
    def test_source_domains_have_disjoint_groups_and_cover_only_training(self):
        ...
        for domain, (fitting, heldout) in source_partitions(train).items():
            ...
            self.assertFalse(
                set(train.prompts.loc[fitting].leakage_group)
                & set(train.prompts.loc[heldout].leakage_group)
            )
        ...
        train.prompts.loc[train.prompts.index[40], "leakage_group"] = (
            train.prompts.leakage_group.iloc[0]
        )
        with self.assertRaises(ValueError):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for domain, (fitting, heldout) in source_partitions(train).items():
                ...
                np.testing.assert_allclose(
                    [metadata["mean_training_cost_usd"][m] for m in MODEL_IDS],
                    train.costs.loc[fitting].mean(),
                )

    def test_ood_evidence_rejects_code_and_test_role(self):
        val = synthetic_part("validation", "ood")
        actions = pd.DataFrame(
            np.full((80, 6), REFERENCE_INDEX), index=val.prompts.index, columns=POLICY_IDS
        )
    ...
    def test_joint_bounds_match_independent_group_resampling_and_maximum(self):
        ...
        for name in ("ordinary", "source_holdout"):
            ...
            normalized = np.divide(
                draws - observed, std, out=np.zeros_like(draws), where=std > 1e-12
            )
        ...
        self.assertEqual(meta["critical_value"], float(np.quantile(np.maximum(*maxima), 0.95)))
        np.testing.assert_array_equal(
            bounds.joint_lower,
            bounds.quality_delta - meta["critical_value"] * bounds.standard_error,
        )
    ...
    def test_runner_freezes_gpt5_and_actions_before_requesting_test_outcomes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def read_checked(data, request_prompts, regime, name):
                if name == "test":
                    ...
                    for filename in (
                        ...
                        "test_actions.parquet",
                        "test_routing_decisions.parquet",
                    ):
            ...
            with (
                patch(
                    "experiments.fixed_reference_router.canonical_part", side_effect=read_checked
                ),
                patch(
                    "experiments.fixed_reference_router.evaluate",
                    return_value={"comparisons": {"primary": {}}},
                ),
            ):
...
class FixedReferenceArtifactTests(unittest.TestCase):
        ...
        for regime in ("standard", "ood"):
            ...
            for domain, rows in assignments.groupby("excluded_domain"):
                ...
                pd.testing.assert_frame_equal(
                    pd.read_parquet(path / "heldout_predictions.parquet").set_index("prompt_id"),
                    model.predict(
                        SimpleNamespace(regime=regime, prompts=self.prompts.loc[heldout.prompt_id])
                    ),
                    check_exact=True,
                )
                np.testing.assert_allclose(
                    [metadata["mean_training_cost_usd"][m] for m in MODEL_IDS],
                    train.costs.loc[fitting.prompt_id].mean(),
                )
    ...
    def test_frozen_selection_matches_all_validation_bounds_and_costs(self):
        for regime in ("standard", "ood"):
            ...
            expected = bounds.groupby("policy_id").joint_lower.min().ge(-0.005)
            self.assertEqual(
                candidates.set_index("policy_id").feasible.to_dict(), expected.to_dict()
            )
            winner = (
                candidates[candidates.feasible]
                .sort_values(
                    ["mean_cost_usd", "quality", "policy_id"], ascending=[True, False, True]
                )
                .iloc[0]
                .policy_id
            )
            ...
            np.testing.assert_array_equal(
                samples.joint_max_t,
                np.maximum(samples.ordinary_max_t, samples.source_holdout_max_t),
            )
            self.assertEqual(
                np.quantile(samples.joint_max_t, 0.95),
                frozen["selection_metadata"]["critical_value"],
            )
            train = canonical_part(DATA, self.prompts, regime, "train")
            np.testing.assert_allclose(
                [frozen["mean_training_cost_usd"][m] for m in MODEL_IDS], train.costs.mean()
            )
    ...
    def test_all_final_predictions_evaluation_and_runtime_align(self):
        for regime in ("standard", "ood"):
            ...
            for split in ("train", "validation", "test"):
                ...
                predictions = pd.read_parquet(path / f"{split}_predictions.parquet").set_index(
                    "prompt_id"
                )
            ...
            decisions = pd.read_parquet(path / "test_routing_decisions.parquet").set_index(
                "prompt_id"
            )
            self.assertFalse(
                {"success", "cost_usd", "dataset", "leakage_group"} & set(decisions.columns)
            )
            ...
            for record in routed:
                if record["probabilities"] is not None:
                    np.testing.assert_array_equal(
                        [record["probabilities"][m] for m in MODEL_IDS],
                        predictions.loc[record["prompt_id"]],
                    )
        ...
        for regime in ("standard", "ood"):
            ...
            for name, c in r["comparisons"].items():
                ...
                for metric, interval in c["bootstrap"]["ci_95"].items():
                    np.testing.assert_array_equal(
                        np.quantile(samples[metric], [0.025, 0.975]), interval
                    )
