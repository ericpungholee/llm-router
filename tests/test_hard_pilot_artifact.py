...
class HardPilotArtifactTests(unittest.TestCase):
    def test_audit_reconstructs_original_and_regrade_only_changes_grading_fields(self):
        provenance = json.loads((ROOT / 'reports/snapshots/hard_pilot_final.provenance.json').read_text())
        snapshot = ROOT / provenance['snapshot_path']
        self.assertEqual(hashlib.sha256(snapshot.read_bytes()).hexdigest(), provenance['final_artifact']['sha256'])
        with snapshot.open(newline='') as handle:
            reader = csv.DictReader(handle)
            fields = reader.fieldnames
            original = list(reader)
        key = lambda r: (r['prompt_id'], r['inference_provider'], r['api_model_identifier'])
        ...
        self.assertEqual(hashlib.sha256(original_bytes).hexdigest(), provenance['before_offline_regrade']['sha256'])
    ...
    def test_fresh_checkout_without_data_reproduces_all_reports(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run([sys.executable, 'pilot_analysis.py', 'reports/snapshots/hard_pilot_final.csv'],
            ...
            subprocess.run([sys.executable, 'reports/reproduce_hard_pilot.py', '--output-dir', 'rebuilt'],
                           cwd=checkout, check=True, capture_output=True, text=True)
            for name in ('hard_pilot_existing_analysis.json', 'hard_pilot_routing_analysis.json', 'hard_pilot_routing_analysis.md'):
                self.assertEqual((checkout / 'rebuilt' / name).read_bytes(), (ROOT / 'reports' / name).read_bytes(), name)
            report = json.loads((checkout / 'rebuilt/hard_pilot_routing_analysis.json').read_text())
            self.assertEqual(report['provenance']['snapshot_path'], 'reports/snapshots/hard_pilot_final.csv')
            self.assertIn(report['provenance']['snapshot_path'], (checkout / 'rebuilt/hard_pilot_routing_analysis.md').read_text())
            self.assertFalse((checkout / 'data').exists())
            ...
            failed = subprocess.run([sys.executable, 'reports/reproduce_hard_pilot.py'],
