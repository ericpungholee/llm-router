...
from routing_ml.embeddings import ENCODER_CONFIG, encode_prompts, file_hash, load_cache, verify_encoder
...
@unittest.skipUnless(ENCODER_AVAILABLE, "Download the pinned local v2 encoder to verify real inference")
class LocalEncoderTests(unittest.TestCase):
    def test_real_encoder_offline_determinism_and_response_projection(self):
        prompts = pd.DataFrame(dict(prompt_id=["a", "b", "c"], prompt=["What is two plus three?", "What is two plus three?", "word "*1100]))
...
class SavedCampaignTests(unittest.TestCase):
    ...
    def test_all_artifact_and_original_data_hashes_match(self):
        ...
        for regime in ("standard", "ood"):
            ...
            for name, expected in json.loads((directory / "artifact_hashes.json").read_text()).items():
    ...
    def test_all_predictions_and_cv_selection_reproduce(self):
        for regime in ("standard", "ood"):
            for family in FAMILIES:
                ...
                scores = pd.read_parquet(directory / "cv_scores.parquet").groupby("C").log_loss.mean()
                self.assertEqual(model.selected_c, min(scores.index, key=lambda c: (scores[c], c)))
                for split in ("train", "validation", "test"):
                    part = read_split(DATA, regime, split)
                    saved = pd.read_parquet(directory / f"{split}_predictions.parquet").set_index("prompt_id")
                    self.assertEqual(tuple(saved.columns), MODEL_IDS)
                    pd.testing.assert_frame_equal(saved, predict_family(model, part, self.embeddings), check_exact=True)
    ...
    def test_saved_primary_is_validation_minimum_feasible(self):
        for regime in ("standard", "ood"):
            ...
            for metric in ("micro", "macro"):
                np.testing.assert_allclose(table[metric + "_lower"], table[metric + "_delta"] - critical * table[metric + "_standard_error"], atol=1e-12)
            feasible = table[(table.micro_lower >= -0.005) & (table.macro_lower >= -0.005) & (table.worst_dataset_delta >= -0.005)]
            winner = min(feasible.itertuples(), key=lambda r: (r.mean_cost_usd, -r.quality, r.candidate_id))
            self.assertEqual(winner.candidate_id, frozen["primary"]["candidate_id"])
            actions = pd.read_parquet(directory / "test_actions.parquet")
            np.testing.assert_array_equal(actions.primary, actions[frozen["primary"]["candidate_id"]])
    ...
    def test_scalar_policy_reference_for_every_candidate_on_test_sample(self):
        for regime in ("standard", "ood"):
            ...
            predictions = {f: pd.read_parquet(directory / f / "test_predictions.parquet").set_index("prompt_id").loc[sample_ids] for f in FAMILIES}
            similarity = pd.read_parquet(directory / "test_novelty.parquet").set_index("prompt_id").similarity
            for policy in policies.itertuples():
                for pid in sample_ids:
                    if policy.kind == "static" or (policy.gate_quantile and similarity[pid] < frozen["novelty_cutoffs"][str(policy.gate_quantile)]):
                        chosen = baseline
                    else:
                        p = predictions[policy.family].loc[pid].to_dict()
                        if policy.kind == "advantage":
                            eligible = [m for m in MODEL_IDS if costs[m] < costs[baseline] and p[m] >= p[baseline] + policy.value]
                            chosen = min(eligible, key=lambda m: (costs[m], -p[m], m)) if eligible else baseline
                        elif policy.kind == "threshold":
                            eligible = [m for m in MODEL_IDS if p[m] >= policy.value]
                            chosen = min(eligible, key=lambda m: (costs[m], -p[m], m)) if eligible else min(MODEL_IDS, key=lambda m: (-p[m], costs[m], m))
                        else:
                            chosen = min(MODEL_IDS, key=lambda m: (-(p[m]-policy.value*costs[m]/max(costs.values())), costs[m], m))
    ...
    def test_all_cost_quality_points_match_source_outcomes(self):
        for regime in ("standard", "ood"):
            ...
            c = test.costs.to_numpy(dtype=float)[np.arange(len(actions))[:, None], actions.to_numpy()]
            summary = pd.read_parquet(directory / "test_policy_metrics.parquet")
            micro = summary[summary.dataset == "__micro__"].set_index("policy_id").loc[actions.columns]
            np.testing.assert_allclose(micro.quality, y.mean(axis=0))
            np.testing.assert_allclose(micro.mean_cost_usd, c.mean(axis=0))
            self.assertFalse({"success", "cost_usd"} & set(pd.read_parquet(directory / "test_routing_decisions.parquet").columns))
    ...
    def test_label_free_local_router_matches_frozen_decisions_and_probabilities(self):
        ...
        for regime in ("standard", "ood"):
            ...
            selected = pd.read_parquet(directory / "test_actions.parquet").set_index("prompt_id")
            self.assertEqual([r["selected_model_id"] for r in actual], [MODEL_IDS[i] for i in selected.primary])
            if router.candidate.family != "static":
                saved = pd.read_parquet(directory / router.candidate.family / "test_predictions.parquet").set_index("prompt_id")
                np.testing.assert_array_equal([[row["probabilities"][m] for m in MODEL_IDS] for row in actual], saved)
