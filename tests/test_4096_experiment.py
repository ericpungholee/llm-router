...
from generate_dataset import read_results, result_key
from hard_pilot_experiment import (PINNED_FILES, PROVENANCE, ROOT, SNAPSHOT, SOURCE,
                                   build_report, freeze, frozen_inputs, grading_records,
                                   select_exhausted, treatment_calls, treatment_plan,
...
class ClosedPilotTests(unittest.TestCase):
    ...
    def test_frozen_accounting_and_oracle(self):
        ...
        self.assertEqual(report["output_exhaustion"]["by_task_type"], {"code": 14, "math": 2, "multiple_choice": 4})
    ...
    def test_freeze_copies_exact_bytes_without_calls_or_source_mutation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(provenance["offline_audit"], {"provider_calls": 0, "blocked_network_attempts": 0})
    ...
    def test_fresh_checkout_reproduces_report_and_preflight_without_data_or_network(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            boot = ("import runpy,sys,socket,urllib.request; "
            ...
            subprocess.run([sys.executable, "-c", boot], cwd=checkout, check=True, capture_output=True, text=True)
            for name in ("hard_pilot_4096_analysis.json", "hard_pilot_4096_analysis.md"):
                self.assertEqual((checkout / "rebuilt" / name).read_bytes(), (ROOT / "reports" / name).read_bytes())
            preflight = subprocess.run([sys.executable, "hard_pilot_treatment.py", "--max-output-tokens", "8192",
                "--max-spend-usd", "1.66", "--output", "data/results/treatment.csv"],
            ...
            failed = subprocess.run([sys.executable, "reports/reproduce_4096_pilot.py", "--output-dir", "rebuilt"],
                cwd=checkout, capture_output=True, text=True)
    ...
    def test_only_explicit_final_exhaustion_is_selected(self):
        ...
        self.assertTrue(all(r["status"] == "provider_error" and r["error_type"] == "output_limit_exhausted" for r in selected))
        self.assertFalse({result_key(r) for r in selected} & {result_key(r) for r in self.rows if r["status"] == "success"})
    ...
    def test_grok_cannot_enter_even_if_it_is_explicitly_exhausted(self):
        rows = copy.deepcopy(self.rows)
        row = next(r for r in rows if r["inference_provider"] == "xai" and r["status"] == "provider_error")
        row["error_type"] = "output_limit_exhausted"
        self.assertNotIn(result_key(row), {result_key(r) for r in select_exhausted(rows)})
    ...
    def test_costs_and_one_attempt_plan(self):
        for budget, cap, expected in ((8192, "1.66", "1.656349"), (16384, "3.22", "3.216105")):
            ...
            self.assertEqual(estimate_max_spend_usd([(m, p, b) for _, m, p, b in calls], max_retries=0), Decimal(expected))
    ...
    def test_paid_command_stops_before_credentials_files_or_calls(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("hard_pilot_treatment.load_dotenv") as credentials, patch("providers.call_model") as caller:
    ...
    def test_mocked_paid_execution_never_retries_exhaustion_or_transient_errors(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            synthetic.update(strict_preflight_passed=True, cost_bound_issues=[],
            ...
            def exhausted(model, prompt, **kwargs):
                seen.append((model.inference_provider, model.api_model_identifier, prompt, kwargs["max_output_tokens"]))
                kind = "output_limit_exhausted" if len(seen) % 2 else "network_error"
                raise ProviderError(model.inference_provider, model.api_model_identifier or "", "exhausted",
                                    error_type=kind, retryable=kind == "network_error")
            with patch("hard_pilot_treatment.prepare", return_value=resolved), \
            ...
            self.assertTrue(all(r["provider_attempts"] == 1 and r["retry_count"] == 0 for r in result))
            ...
            self.assertEqual({result_key(r) for r in result}, {result_key(r) for r in select_exhausted(self.rows)})
            with self.assertRaises(FileExistsError):
