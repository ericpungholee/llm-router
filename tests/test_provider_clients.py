...
class ProviderAdapterTests(unittest.TestCase):
    def test_http_billing_normalization_preserves_scope(self):
        for message, scope in (
            ("Insufficient credits", "provider"),
            ("Requires fewer max_tokens", "model"),
        ):
    ...
    def test_billing_exhaustion_and_zero_resource_quota_have_different_scopes(self):
        cases = [
            ...
            (
                402,
                "",
                "This request requires more credits, or fewer max_tokens; can only afford 100 tokens",
                "model",
            ),
    ...
    def test_openai_responses_adapter(self):
        ...
        self.assertEqual(
            (response.text, response.input_tokens, response.output_tokens), ("B", 10, 2)
        )
    ...
    def test_anthropic_messages_adapter(self):
        ...
        self.assertEqual(
            (response.text, response.input_tokens, response.output_tokens), ("B", 11, 3)
        )
    ...
    def test_xai_responses_adapter(self):
        ...
        self.assertEqual(
            (response.text, response.input_tokens, response.output_tokens), ("B", 13, 5)
        )
    ...
    def test_deepseek_chat_adapter(self):
        ...
        self.assertEqual(
            (response.text, response.input_tokens, response.output_tokens), ("B", 14, 6)
        )
    ...
    def test_openrouter_chat_adapter_keeps_reported_cost(self):
        ...
        self.assertEqual(
            (response.text, response.input_tokens, response.output_tokens), ("B", 15, 7)
        )
