...
def render(r):
    ...
    table(["Metric", "Value"], [["Rows / expected pairs", f"{m['recorded_pairs']} / {m['expected_pairs']}"],
        ...
        ["Skipped / spend stops / unrecorded", f"{m['skipped_pairs']} / {m['spend_limit_stops']} / {m['unrecorded_pairs']}"],
    ...
    add(f"The existing response counter excludes {p['http_200_exhausted_responses']} saved HTTP-200 exhaustion outcomes; those are provider-returned responses but failed normalized completions. The 83 attempted calls include earlier attempts/resumes, with some history inferred from legacy fields. Exhausted/unknown billed failures carry maximum-cost reserves. Recorded spend is a ledger estimate, not a reconciled invoice; see the historical billing limitations in [xAI cost safety](xai_cost_safety.md). All remaining failure/skipped states remain missing grades, never incorrect answers.")
    add()
    headers = ["Group", "Graded / expected", "Correct / graded", "Provider fail", "Parse / grade fail", "Skipped", "Spend stop", "Spend USD"]
    def metric_row(label, v):
        return [label, f"{v['successfully_graded_pairs']} / {v['expected_pairs']}",
                f"{v['correct_responses']} / {v['successfully_graded_pairs']}", v["provider_failures"],
                f"{v['parsing_failures']} / {v['grading_failures']}", v["skipped_pairs"],
                v["spend_limit_stops"], f"{v['total_recorded_cost_usd']:.7f}"]
    ...
    add("```bash\n" + command + " --confirm\n```")
    add()
    add("The 16384 alternative uses `--max-output-tokens 16384 --max-spend-usd 3.22 --output data/results/hard_pilot_exhaustion_16384.csv`; it is also blocked. No paid treatment has run. The test suite exercises only mocked paid paths and blocks real HTTP/socket access.")
    add()
    return "\n".join(lines)
...
def main():
    ...
    parser.add_argument("--freeze", action="store_true", help="Exclusive creation; never refreeze/overwrite")
    ...
    with offline_only():
        if args.freeze:
            freeze()
        report = build_report()
        content = {"hard_pilot_4096_analysis.json": json.dumps(report, indent=2, sort_keys=True)+"\n",
                   "hard_pilot_4096_analysis.md": render(report)}
        # A reproduction destination cannot replace any pinned experimental input.
        protected = {(ROOT / SOURCE).resolve(), (ROOT / SNAPSHOT).resolve(),
                     (ROOT / SNAPSHOT.with_suffix(".provenance.json")).resolve()}
        if any((args.output_dir / name).resolve() in protected or
               (args.output_dir / name).resolve().is_relative_to((ROOT / "reports/snapshots").resolve())
               for name in content):
