...
from generate_dataset import is_completed_result, read_results, saved_model_blocks
...
RESUME = (
    "python3 generate_dataset.py --hard-pilot --resume --max-spend-usd 2.00 "
...
def disagreement(rows, prompts, models):
    ...
    return dict(
        ...
        all_correct=correct,
        all_wrong=wrong,
        all_agree=correct + wrong,
        disagree=disagree,
        rate=disagree / len(common) if common else None,
    )
...
def static_metrics(rows, common, models):
    ...
    for m in models:
        ...
        result[model_label(m)] = dict(
            ...
            correct_per_dollar=float(Decimal(str(accuracy)) * len(selected) / total)
            if total
            else None,
        )
    return result
...
def classification(row):
    if row["error_type"] == "spend_limit_error":
        return "spend-limit stop"
    if row["status"] == "skipped_model":
        return "skipped historical row"
    if row["status"] == "parsing_failure":
        return "parsing failure"
    if row["status"] == "grading_failure":
        return "grading failure"
...
def build_report():
    ...
    for p, b in membership.items():
        values = {
            model_label(m): matrix[p, m]["correct"]
            if (p, m) in matrix and matrix[p, m]["status"] == "success"
            else None
            for m in models
        }
        vectors.append(
            dict(
                ...
                correctness=values,
                statuses={
                    model_label(m): matrix[p, m]["status"] if (p, m) in matrix else "unrecorded"
                    for m in models
                },
                confirmed_disagreement={v for v in values.values() if v is not None}
                == {True, False},
            )
        )
    pairwise = [
        {
            ...
            "subsets": {
                name: disagreement(rows, ps, [a, b])
                for name, ps in [("all", list(membership))] + list(plan.items())
            },
        }
    ...
    incomplete = [
        dict(
            ...
            has_saved_raw_response=bool(r["raw_response"].strip()),
            recoverable_offline=False,
            requires_provider_call=not bool(r["raw_response"].strip()),
        )
        for r in rows
        if r["status"] != "success"
    ]
    ...
    secondary["static_baselines"] = static_metrics(
        secondary_rows, secondary_common, secondary_models
    )
    ...
    fallback_rows = [
        ...
        if r["prompt_id"] in comparison["oracle_unsolved_prompt_ids"]
        and model_label(model_key(r)) == fallback_label
    ]
    full_cost = Decimal(str(comparison["oracle_cheapest_correct_cost_usd"])) + sum(
        (response_cost(r) for r in fallback_rows), Decimal(0)
    )
    comparison["oracle_cost_with_cheapest_static_fallback_usd"] = float(full_cost)
    comparison["oracle_full_cost_relative_to_best_static"] = float(
        full_cost / Decimal(str(comparison["always_best_single_model"]["total_response_cost_usd"]))
    )
    comparison["oracle_accuracy_lift"] = (
        comparison["oracle_accuracy"] - comparison["always_best_single_model"]["accuracy"]
    )
    static = static_metrics(rows, common, models)
    cost_static = [
        (v["correct_per_dollar"], k)
        for k, v in static.items()
        if v["correct_per_dollar"] is not None
    ]
    return dict(
        provenance=provenance,
        existing_analysis=base,
        overall=_metrics(rows, 75),
        by_benchmark={
            b: _metrics([r for r in rows if r["benchmark"] == b], len(ps) * len(models))
            for b, ps in plan.items()
        },
        vectors=vectors,
        five_model_disagreement=disagreement(rows, list(membership), models),
        by_benchmark_disagreement={b: disagreement(rows, ps, models) for b, ps in plan.items()},
        confirmed_disagreement_lower_bound={
            "count": sum(v["confirmed_disagreement"] for v in vectors),
        ...
        five_model_oracle={
            "accuracy": base["routing_comparison"]["oracle_accuracy"],
            "lift_over_best_single": None,
            "cheapest_correct_cost_usd": base["routing_comparison"][
                "oracle_cheapest_correct_cost_usd"
            ],
            "cost_relative_to_best_static": None,
            "reason": "No fully graded five-model prompts",
        },
        incomplete_pairs=incomplete,
        incomplete_classifications=dict(Counter(r["classification"] for r in incomplete)),
        resume={
            "preflight_command": RESUME,
            "authorized_later_resume_command": RESUME + " --confirm",
            ...
            "maximum_attempts_for_eligible_pairs": 3 * len(incomplete),
            "executed": False,
        },
        secondary_gpt_claude=secondary,
        decision={
            "ml": "defer",
            "routing_could_help": "Five-model complementarity unmeasurable. GPT/Claude accuracy shows static Claude dominance; a cost oracle suggests limited potential savings only.",
...
def render(report):
    ...
    def table(headers, rows):
        ...
        for row in rows:
            add(
                "| " + " | ".join(fmt(v).replace("|", "\\|").replace("\n", " ") for v in row) + " |"
            )
        add()

    def money(x):
        return "N/A" if x is None else f"${x:.7f}"
    ...
    p = report["provenance"]
    m = report["overall"]
    base = report["existing_analysis"]
    add("# Final hard-pilot routing analysis")
    add()
    add(
    ...
    add()
    add("**Final artifact and provenance**")
    add()
    add(
        f"Source final CSV: `{p['source_final_csv']}`. Primary analysis input: [`{SNAPSHOT}`](snapshots/hard_pilot_final.csv). Frozen at {p['frozen_at_utc']}, from working tree based on commit `{p['base_commit']}`. The input CSV is included in the repository; no `data/results/` files or credentials are needed for reproduction."
    )
    add()
    add(
        f"Final SHA-256: `{p['final_artifact']['sha256']}`. File size: {p['final_artifact']['size_bytes']:,} bytes; row count: {p['final_artifact']['row_count']}."
    )
    add()
    obs = p["stability"]["observations"]
    add(
        f"Stability evidence: process inspection found no Python or hard-pilot writer. Identical size, SHA-256, row count and modification time were observed at {obs[0]['observed_at']} and {obs[1]['observed_at']}. No original exit log/status was available, so successful exit is not asserted. Before regrade the source was {p['before_offline_regrade']['size_bytes']:,} bytes, SHA-256 `{p['before_offline_regrade']['sha256']}`."
    )
    add()
    add(
    ...
    add()
    add("**One offline recovery and complete change audit**")
    add()
    add(
        "Ran once against the stable source: `"
        + p["offline_regrade"]["command"]
        + "`. The existing standalone bold-option parser remains unchanged and narrow. Claude’s saved `**F**` on `mmlu_pro:12015` becomes parsed `F`, matching reference `F`. Raw responses and all provider telemetry, costs, latencies, tokens, timestamps, request IDs and generation settings were verified unchanged."
    )
    add()
    table(
        ...
        [
            [
                r["prompt_id"] + " / " + r["api_model_identifier"],
                field,
                repr(v["before"]),
                repr(v["after"]),
            ]
            for r in p["offline_regrade"]["changed_rows"]
    ...
    table(
        ...
        [[key, p["before_offline_regrade"]["metrics"][key], m[key]] for key in keys],
    )
    add(
    ...
    table(
        headers,
        [
            [b + " / " + short(k)] + [v[x] for x in groupkeys]
            for b, vs in base["by_benchmark_and_model"].items()
            for k, v in vs.items()
        ],
    )
    add("**Five-model routing signal**")
    add()
    add(
    ...
    table(
        ["Prompt"] + [short(x) for x in model_labels],
        [
            [v["prompt_id"]]
            + [
                int(v["correctness"][x])
                if v["correctness"][x] is not None
                else {"provider_error": "F", "skipped_model": "S"}.get(v["statuses"][x], "?")
    ...
    table(
        ...
        [
            [name]
            + [v[k] for k in ["n", "all_correct", "all_wrong", "all_agree", "disagree", "rate"]]
            for name, v in [("All five models", d)]
            + list(report["by_benchmark_disagreement"].items())
        ],
    )
    add(
    ...
    table(
        ...
        [
            [" / ".join(short(x) for x in pair["models"])]
            + [
                f"{v['disagree']}/{v['n']} ({v['rate']:.0%})" if v["n"] else "N/A (n=0)"
                for v in [
                    pair["subsets"][b] for b in ["all", "mmlu_pro", "math_500", "livecodebench"]
                ]
            ]
            for pair in report["pairwise_disagreement"]
        ],
    )
    add("**Static policies and oracle on the five-model common subset**")
    add()
    table(
        ...
        [
            [
                ...
                v["n"],
                v["accuracy"],
                money(v["total_cost_usd"]),
                money(v["average_cost_usd"]),
                v["average_latency_ms"],
            ]
            for k, v in report["static_baselines"].items()
        ],
    )
    add(
    ...
    add()
    add("**Secondary diagnostic: GPT/Claude only, ten shared graded prompts**")
    add()
    s = report["secondary_gpt_claude"]
    sc = s["routing_comparison"]
    table(
        ...
        [
            ...
            for k, v in s["static_baselines"].items()
        ],
    )
    add(
    ...
    add()
    add(
        f"The reduced oracle is {sc['oracle_accuracy']:.0%}, with {sc['oracle_accuracy_lift']:.0%} accuracy lift over Claude. Cheapest-correct cost is {money(sc['oracle_cheapest_correct_cost_usd'])} for eight solved prompts only. The two unsolved prompts are `mmlu_pro:5022` and `mmlu_pro:6502`. Charging a Claude fallback on those gives {money(sc['oracle_cost_with_cheapest_static_fallback_usd'])} across all ten, {sc['oracle_full_cost_relative_to_best_static']:.1%} of Claude’s total cost (15.3% savings at equal accuracy). This uses ex-post correctness and realized costs; it does not demonstrate a prompt-visible policy or generalization."
    )
    add()
    add("**Why the matrix is incomplete and what resume would do**")
    add()
    add(
    ...
    table(
        ...
        [
            [
                ...
                short(r["model"]),
                r["classification"],
                r["error_type"] + ": " + r["reason"],
            ]
            for r in report["incomplete_pairs"]
        ],
    )
    add(
    ...
    add()
    add("Preflight only (no provider calls; not executed here):")
    add()
    add("```bash\n" + RESUME + "\n```")
    add()
    add("After explicit later authorization to spend, the exact resume command is:")
    add()
    add("```bash\n" + RESUME + " --confirm\n```")
    add()
    add(
    ...
    add()
    add("**Decision and reproducibility**")
    add()
    add(
    ...
    add()
    add("Test verification: " + json.dumps(p["verification"], sort_keys=True) + ".")
    add()
    add(
...
def main():
    ...
    for name, value in [
        ("hard_pilot_existing_analysis.json", report["existing_analysis"]),
        ("hard_pilot_routing_analysis.json", report),
    ]:
