...
PINNED_FILES = ("model_registry.py", "benchmarks/manifest.json", "benchmarks/hard_pilot.json",
                "benchmark_loaders.py", "graders.py", "generate_dataset.py", "providers.py",
                "provider_clients/base.py", "provider_clients/normalization.py",
                "provider_clients/openai.py", "provider_clients/anthropic.py",
                "provider_clients/deepseek.py", "provider_clients/openrouter.py",
                "benchmarks/samples/mmlu_pro.jsonl", "benchmarks/samples/math_500.jsonl",
...
def freeze(root=ROOT):
    ...
    with offline_only() as audit:
        ...
        data = {
            "experiment": "hard_pilot_4096_final", "source_path": str(SOURCE),
            "snapshot_path": str(SNAPSHOT), "source_sha256": source_hash,
            "frozen_sha256": frozen_hash, "row_count": len(rows),
            "size_bytes": len(original), "recorded_spend_usd": exact_spend(rows),
            "timestamp_utc": datetime.now(timezone.utc).isoformat(), "git_commit": commit,
            "metrics": metrics,
            "output_limit_exhausted_count": sum(r["error_type"] == "output_limit_exhausted" for r in rows),
            "http_200_exhausted_responses": sum(
                d.get("http_status") == 200 and d.get("outcome") == "output_limit_exhausted"
                for r in rows for d in json.loads(r.get("provider_diagnostics") or "[]")),
            ...
            "observed_generation_configuration": [
                {"provider": p, "model_id": mid, "temperature": t,
                 "reasoning_setting": effort, "max_output_tokens": budget}
                for p, mid, t, effort, budget in sorted({
                    (r["inference_provider"], r["api_model_identifier"], r["temperature"],
                     r["reasoning_setting"], r["max_output_tokens"]) for r in rows})],
            ...
            "source_unchanged": True, "no_provider_calls_while_freezing": True,
...
def frozen_inputs(root=ROOT, *, verify_configuration=False):
    ...
    validate_results(rows)
    if len(rows) != provenance["row_count"] or exact_spend(rows) != provenance["recorded_spend_usd"]:
    ...
    models = tuple(ModelConfig(**{**c, "generation": GenerationSettings(**c["generation"])})
                   for c in provenance["generation_configuration"] if c["enabled"])
...
def treatment_calls(rows, models, budget):
    ...
    for row in select_exhausted(rows):
        ...
        if (row["max_output_tokens"] != 4096 or row["reasoning_setting"] != model.generation.reasoning_setting
    ...
    for row in rows:
        ...
        for diagnostic in json.loads(row.get("provider_diagnostics") or "[]"):
            ...
            if isinstance(output, int) and isinstance(limit, int) and output > limit:
                issues.append({"prompt_id": row["prompt_id"], "model": model_label(model_key(row)),
                               "requested_max_output_tokens": limit, "output_tokens": output,
...
def validate_treatment_output(output, root=ROOT):
    ...
    for path in (output, output.with_suffix(output.suffix + ".tmp"),
        ...
        if path.exists() or path.is_symlink():
            raise FileExistsError("Treatment output/checkpoint already exists; no overwrite or paid resume")
    return output
...
def treatment_plan(rows, models, budget, spend_cap="1.66"):
    ...
    return {
        "experiment": f"hard_pilot_exhaustion_{budget}", "parent_snapshot": str(SNAPSHOT),
        "max_output_tokens": budget, "planned_calls": len(calls),
        "max_retries": 0, "maximum_attempts_per_pair": 1,
        ...
        "spend_cap_usd": str(plan.configured_spend_cap_usd), "within_cap": cap_ok,
        ...
        "by_model": { label: {
            ...
            "registry_contract_maximum_usd": str(sum(
                (max_call_cost_usd(m, prompt, b) for r, m, prompt, b in calls
                 if model_label(model_key(r)) == label), Decimal(0)))}
            for label in sorted(selected_models)},
        "pairs": [{"prompt_id": r["prompt_id"], "model": model_label(model_key(r)),
                   "benchmark": r["benchmark"], "task_type": r["task_type"],
...
def pair_comparison(rows, a, b, prompts):
    ...
    common = [p for p in sorted(prompts) if all((p, m) in matrix and matrix[p, m]["status"] == "success" for m in (a, b))]
    ...
    return {"models": [model_label(a), model_label(b)], "n": len(common), "prompt_ids": common,
            "accuracy_a": sum(matrix[p, a]["correct"] for p in common)/len(common) if common else None,
            "accuracy_b": sum(matrix[p, b]["correct"] for p in common)/len(common) if common else None,
            "a_only_correct": wins_a, "b_only_correct": wins_b, "both_correct": both_correct,
            "both_wrong": len(common)-wins_a-wins_b-both_correct,
            "disagreement": wins_a+wins_b, "disagreement_rate": (wins_a+wins_b)/len(common) if common else None}
...
def build_report(root=ROOT):
    with offline_only() as audit:
        ...
        for m in keys:
            ...
            static[model_label(m)] = {"n": len(selected), "correct": sum(r["correct"] for r in selected),
        ...
        diagnostics = []
        for r in exhausted:
            ds = [d for d in json.loads(r.get("provider_diagnostics") or "[]") if d.get("outcome") == "output_limit_exhausted"]
            d = ds[-1] if ds else {}
            diagnostics.append({"prompt_id": r["prompt_id"], "model": model_label(model_key(r)),
                "benchmark": r["benchmark"], "task_type": r["task_type"],
                "requested_max_output_tokens": d.get("requested_max_output_tokens", r["max_output_tokens"]),
                "output_tokens": d.get("output_tokens", r["output_tokens"]),
                "reasoning_tokens": d.get("reasoning_tokens"), "stop_reason": d.get("stop_reason", r["stop_reason"]),
                "incomplete_reason": d.get("incomplete_reason"),
                "visible_text_present": d.get("visible_text_present", bool(r["raw_response"].strip()))})
        potential = {}
        for name, subset in [("five_model", keys), ("four_non_xai_models", [m for m in keys if m[0] != "xai"])]:
            potential[name] = [p for p in sorted(membership) if all(
                matrix[p, m]["status"] == "success" or result_key(matrix[p, m]) in {result_key(r) for r in exhausted}
                for m in subset)]
        pairs = [{"all": pair_comparison(rows, a, b, membership),
                  "by_benchmark": {bench: pair_comparison(rows, a, b, ps) for bench, ps in plan.items()}}
                 for a, b in combinations(keys, 2)]
        return {"provenance": provenance, "analysis": base, "overall": _metrics(rows, 75),
                "recorded_spend_usd": exact_spend(rows),
                "by_benchmark": {b: _metrics([r for r in rows if r["benchmark"] == b], len(ps)*len(keys)) for b, ps in plan.items()},
                "common_vectors": [{"prompt_id": p, "benchmark": membership[p],
                                    "correctness": {model_label(m): matrix[p, m]["correct"] for m in keys}} for p in common],
                "static_baselines": static, "pairwise": pairs,
                "disagreement_matrix": {model_label(a): {model_label(b): pair_comparison(rows, a, b, membership) for b in keys} for a in keys},
                "common_disagreement_count": sum(len({matrix[p, m]["correct"] for m in keys}) > 1 for p in common),
                "output_exhaustion": {"count": len(exhausted), "pairs": diagnostics,
                    "by_model": {model_label(m): sum(model_key(r) == m for r in exhausted) for m in keys},
                    "by_benchmark": dict(Counter(r["benchmark"] for r in exhausted)),
                    "by_task_type": dict(Counter(r["task_type"] for r in exhausted)),
                    "by_model_benchmark_task": [{"model": m, "benchmark": b, "task_type": t, "count": n}
                        for (m, b, t), n in sorted(Counter((model_label(model_key(r)), r["benchmark"], r["task_type"]) for r in exhausted).items())]},
                "cost_bound_issues": cost_bound_issues(rows),
                "treatments": {str(b): treatment_plan(rows, models, b, "1.66" if b == 8192 else "3.22")[0] for b in (8192, 16384)},
                "maximum_completeness_after_all_selected_recover": potential,
                "decision": {"suitable_for_learned_router_training_or_evaluation": False,
...
def grading_records(root, rows):
    ...
    for row in rows:
        ...
        reference = record.reference_answer if record.task_type != "code" else json.dumps(list(record.tests), sort_keys=True)
        if (render_prompt(record) != row["prompt"] or record.task_type != row["task_type"]
                or record.benchmark_name != row["benchmark"] or reference != row["reference_answer"]):
