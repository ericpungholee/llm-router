...
PATH_MODEL_EXCEPTIONS = {
    "bench-release/swe-bench/verified/qwen3-235b-a22b-thinking/SWE-Bench-verified_qwen3-235b-a22b-thinking-2507-20251026_124335.json":
        ("swe-bench", "verified", "qwen3-235b-a22b-thinking-2507")
}
...
def download_archive(raw_dir, config, download=False):
    ...
    if not path.exists():
        if not download:
            raise FileNotFoundError(f"{path} missing; pass --download to fetch the pinned public archive")
...
def output_failure(raw):
    ...
    return text.startswith(("processing failed:", "generation failed:", "failed to generate", "error:"))
...
def read_archive(archive, task_config, models):
    ...
    with tarfile.open(archive, "r|gz") as bundle:
        for member in bundle:
            ...
            if len(parts) not in (4, 5) or parts[0] != "bench-release" or not member.name.endswith(".json"):
            ...
            if len(parts) == 4 and ds not in {"arenahard", "arenahard_coding", "arenahard_math", "arenahard_creative_writing"}:
            ...
            for record in data["records"]:
                ...
                if cost is not None:
                    cost_sum += cost
                row = dict(dataset=ds, source_split=split, model_id=model, source_index=index,
                           prompt_sha256=digest(prompt), origin_sha256=digest(origin),
                           raw_score=record.get("score"), raw_cost_usd=cost,
                           input_tokens=record.get("prompt_tokens"), output_tokens=record.get("completion_tokens"),
                           has_output=bool(record.get("raw_output")), generation_failure=output_failure(record.get("raw_output")),
                           source_file=member.name, demo=bool(data.get("demo", False)))
                rows.append(row)
                if (ds, split) in tasks and model in for_model and not row["demo"]:
                    selected.append(dict(row, prompt=prompt, origin_query=origin,
                                         prompt_id=f"llmrouterbench:{ds}:{row['prompt_sha256']}",
                                         task_type=tasks[ds, split]["task_type"]))
            files.append(dict(source_file=member.name, dataset=ds, source_split=split, model_id=model,
                              path_identity_consistent=path_agrees,
                              bytes=member.size, sha256=hashlib.sha256(payload).hexdigest(),
                              records=len(data["records"]), reported_counts=data["counts"], demo=bool(data.get("demo", False)),
                              data_fingerprint=data.get("data_fingerprint"), header_cost_usd=data.get("cost"),
    ...
    stats = dict(record_fields=dict(sorted(fields.items())), header_fields=dict(sorted(header_fields.items())),
