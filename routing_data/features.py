        ...
        zip(
            ...
            [
                ...
                int(
                    bool(
                        re.search(
                            r"```|\b(?:def |class |import |SELECT |function\s*\(|#include)", prompt
                        )
                    )
                ),
                int(
                    bool(
                        re.search(
                            r"\\(?:frac|sqrt|sum|int|boxed)|\$|[=^∑∫√]|\d\s*[+*/<>]\s*\d", prompt
                        )
                    )
                ),
...
def feature_table(prompts):
    ...
    rows = [
        dict(prompt_id=pid, **extract_prompt_features(prompt))
        for pid, prompt in prompts[["prompt_id", "prompt"]].itertuples(index=False, name=None)
    ]
    return (
        pd.DataFrame(rows, columns=["prompt_id"] + FEATURE_COLUMNS)
