...
def validate_pairs(rows):
    ...
    if rows.duplicated(["prompt_id", "model_id"]).any():
        raise ValueError("Duplicate prompt/model pair; no outcome averaging or implicit deduplication")
...
def normalize_outcomes(rows):
    ...
    if (~scores.dropna().isin([0, 1])).any():
        raise ValueError("Selected binary task contains nonbinary score; define a task rule explicitly")
    ...
    out["unavailable_reason"] = [";".join(name for name, bad in [
        ("missing_or_failed_label", not label), ("missing_or_invalid_cost", not cost),
        ("missing_or_invalid_tokens", not tokens)] if bad)
        for label, cost, tokens in zip(out.label_available, out.cost_available, out.tokens_available)]
    return out
...
def coverage_table(rows, prompts, models, group_columns):
    ...
    grid = grid.merge(rows[["prompt_id", "model_id", "eligible", "label_available", "cost_available", "tokens_available"]],
                      how="left", on=["prompt_id", "model_id"], validate="one_to_one", indicator=True)
    ...
    summary = grid.groupby(group_columns).agg(expected_pairs=("prompt_id", "size"),
        present_pairs=("outcome_present", "sum"), eligible_pairs=("eligible", "sum"),
        labeled_pairs=("label_available", "sum"), priced_pairs=("cost_available", "sum"),
        valid_token_pairs=("tokens_available", "sum")).reset_index()
...
def validate_processed(outcomes, prompts, features, models):
    ...
    if set(outcomes.prompt_id) != set(prompts.prompt_id) or set(features.prompt_id) != set(prompts.prompt_id):
    ...
    for col in ("input_tokens", "output_tokens"):
        if outcomes[col].isna().any() or (outcomes[col] <= 0).any() or (outcomes[col] % 1 != 0).any():
            raise ValueError("Invalid processed tokens")
    if list(features.columns) != ["prompt_id"] + FEATURE_COLUMNS or features.prompt_id.duplicated().any():
