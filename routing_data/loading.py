...
def load_split(directory, regime="standard", split="train"):
    ...
    outcomes = pd.read_parquet(directory / "outcomes.parquet", columns=["prompt_id", "model_id", "success", "cost_usd"])
    y = outcomes.pivot(index="prompt_id", columns="model_id", values="success").reindex(index=prompts.index, columns=models)
    costs = outcomes.pivot(index="prompt_id", columns="model_id", values="cost_usd").reindex(index=prompts.index, columns=models)
