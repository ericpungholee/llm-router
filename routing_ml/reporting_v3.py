...
def figures(report, output, directory):
    ...
    axes[0].errorbar(
        q,
        x,
        xerr=np.maximum(0, np.vstack([q - ci[:, 0], ci[:, 1] - q])),
        fmt="o",
        color="#2166ac",
        capsize=3,
    )
    ...
    axes[0].axvline(-0.5, color="#b2182b", ls="--", lw=1, label="−0.5 pp margin")
    ...
    for regime, color, marker in (("standard", "#2166ac", "o"), ("ood", "#b2182b", "s")):
        ...
        axes[1].scatter(
            [f["cost_savings"] * 100 for f in r["folds"]],
            [f["quality_delta"] * 100 for f in r["folds"]],
            color=color,
            marker=marker,
            alpha=0.65,
            s=38,
            label=f"{regime.upper()} folds",
        )
        axes[1].scatter(
            100 * r["primary"]["cost_savings"],
            100 * r["primary"]["quality_delta"],
            color=color,
            marker="*",
            s=160,
            edgecolor="black",
            linewidth=0.5,
            label=f"{regime.upper()} pooled",
        )
...
def generate_report(output, reports):
    ...
    for regime in ("standard", "ood"):
        ...
        p = r["primary"]
        lines += [
            f"**{regime.upper()}:** {pc(r['micro']['router']['quality'])} quality, "
    ...
    lines += [
        "**The standard gain survives this check; OOD routing does not.** "
    ...
    lines += [
        f"MMLU-Pro quality decreases by {pp(mmlu['quality_delta'])} pp, with descriptive CI "
    ...
    lines += [
        "The comparison evaluates the v2 selected decision rules with new, separate grouped partitions. "
    ...
    for regime in ("standard", "ood"):
        ...
        lines += [
            f"## {regime.upper()} pooled held-out result",
            "",
            ...
            md_table(
                ...
                [
                    [
                        pid,
                        pc(r["micro"][pid]["quality"]),
                        f"${r['micro'][pid]['mean_cost_usd']:.8f}",
                        pc(r["macro"][pid]["quality"]),
                    ]
                    for pid in (
                        "router",
                        "best_single",
            ...
            md_table(
                ...
                [[k, str(v)] for k, v in r["descriptive_checks"].items()],
            ),
            "",
            ...
            md_table(
                ...
                [
                    [
                        f["fold"],
                        " / ".join(
                            str(f["split_counts"][k]) for k in ("train", "validation", "test")
                        ),
                        f["best_single"],
                        pc(f["router"]["quality"]),
                        pp(f["quality_delta"]),
            ...
            md_table(
                ...
                [
                    [
                        ...
                        d["n"],
                        pc(d["router"]["quality"]),
                        pc(d["best_single"]["quality"]),
                        pp(d["quality_delta"]),
                        interval(d["bootstrap"]["ci_95"]["quality_delta"]),
                        pc(d["cost_savings"]),
                    ]
            ...
            md_table(
                ...
                [
                    [
                        ", ".join(NAMES[d] for d in row["excluded"]),
                        row["n"],
            ...
            md_table(
                ...
                [
                    [
                        ...
                        else pc(row["mean_predicted_success"]),
            ...
            md_table(
                ...
                [
                    [row["model_id"], f"{row['prevalence']:.4f}"]
                    + [
                        f"{row[k]:.4f}" if row[k] is not None else "—"
                        for k in (
                            "roc_auc",
                            "pr_auc",
                            "log_loss",
        ...
        recovered = (
            "undefined" if r["oracle_gap_recovered"] is None else pc(r["oracle_gap_recovered"])
        )
        lines += [
            f"The router's descriptive Pareto membership against the eight static models and fixed references is "
    ...
    if test_path.exists():
        ...
        lines += [
            f"Full offline suite: **{tests['tests_run']} tests; {tests['failures']} failures, "
    ...
    (reports / "router_v3_results.json").write_text(
