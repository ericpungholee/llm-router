...
class LocalRouter:
    """Load trusted local artifacts only: joblib files can execute Python code."""
    ...
    def __init__(self, artifact_dir):
        ...
        reference = (
            self.frozen["reference"] if "reference" in self.frozen else self.frozen["best_single"]
        )
    ...
    def predict(self, prompts):
        if (
            not prompts.columns.is_unique
            or not {"prompt_id", "prompt"}.issubset(prompts.columns)
            or not prompts.prompt_id.map(lambda x: isinstance(x, str) and bool(x.strip())).all()
            or prompts.prompt_id.duplicated().any()
        ):
        ...
        if self.candidate.family == "static":
            return [
                dict(
                    prompt_id=pid,
                    selected_model_id=MODEL_IDS[self.baseline_index],
                    estimated_cost_usd=float(self.costs[self.baseline_index]),
                    probabilities=None,
                    policy_id=self.candidate.candidate_id,
                    novelty_fallback=False,
                )
        ...
        if family.startswith("embedding") or self.gate is not None:
            embedding, _, _ = encode_prompts(
                prompts, self.directory.parent / "encoder", progress=None
            )
        if family == "tfidf":
            x = self.model.preprocessor.transform(prompts.prompt.tolist())
        else:
            features = (
                [extract_prompt_features(text) for text in prompts.prompt]
                if family in {"handcrafted", "embedding_handcrafted"}
                else None
            )
            numeric = (
                np.asarray(
                    [[row[column] for column in FEATURE_COLUMNS] for row in features], dtype=float
                )
            ...
            raw = (
                ...
                else np.column_stack([embedding, numeric])
                if family == "embedding_handcrafted"
                else embedding
            )
            x = self.model.preprocessor.transform(np.asarray(raw, dtype=float))
        p = pd.DataFrame(
            probabilities(self.model.predictors, x), index=prompts.prompt_id, columns=MODEL_IDS
        )
        similarity = self.gate.score(embedding) if self.gate is not None else np.ones(len(prompts))
        action = candidate_actions(
            {family: p}, self.costs, self.baseline_index, similarity, self.gate, [self.candidate]
        )[:, 0]
        return [
            dict(
                prompt_id=pid,
                selected_model_id=MODEL_IDS[j],
                estimated_cost_usd=float(self.costs[j]),
                probabilities={m: float(p.iloc[i, k]) for k, m in enumerate(MODEL_IDS)},
                policy_id=self.candidate.candidate_id,
                novelty_fallback=bool(
                    self.gate is not None
                    and similarity[i] < self.gate.cutoffs[self.candidate.gate_quantile]
                ),
            )
