...
def figures(frontier, reliability, diagnostics, directory, prefix):
    ...
    for starts, name, color, marker in [
        ("threshold_", "Threshold grid", "#0072B2", "o"),
        ("utility_", "Utility grid", "#D55E00", "^"),
    ]:
    ...
    for r in static.itertuples():
        ...
        ax.annotate(
            label,
            (r.mean_cost_usd * 1000, r.quality * 100),
            xytext=offset,
            textcoords="offset points",
            fontsize=8,
        )
    for pid, name, color, marker in [
        ("deployable_router", "Validation-selected router", "#009E73", "*"),
        ("oracle", "Oracle (hindsight)", "#CC79A7", "D"),
    ]:
        r = frontier[frontier.policy_id == pid].iloc[0]
        ax.scatter(
            r.mean_cost_usd * 1000,
            r.quality * 100,
            s=190 if pid == "deployable_router" else 65,
            facecolor=color,
            edgecolor="black",
            linewidth=0.6,
            marker=marker,
    ...
    for model_id, ax in zip(list(MODEL_IDS) + ["__selected_action__"], axes.flat):
        ...
        ax.scatter(
            r.mean_probability, r.observed_success, s=12 + np.sqrt(r["count"]) * 3, color="#0072B2"
        )
        ax.set(
            title=NAMES.get(model_id, "Selected action"),
            xlim=(0, 1),
            ylim=(0, 1),
            xticks=[0, 0.5, 1],
            yticks=[0, 0.5, 1],
...
def write_report(results, path):
    ...
    verdict = (
        "TF-IDF routing reduced cost with supported noninferiority within 0.5 percentage points."
        if supported and saved
        else "TF-IDF routing did not establish a meaningful cost reduction at statistically preserved best-single quality."
    )
    lines = [
    ...
    for key, heading in [
        ("standard_tfidf", "Standard test"),
        ("ood_tfidf", "Independent OOD test"),
    ]:
        ...
        lines += [
            f"## {heading}",
            "",
            f"Selected C = **{r['selected_C']:g}**; deployable policy = **{r['selected_policy']['policy_id']}**; best-single = **{r['best_single']['model_id']}**; test N = {r['split_counts']['test']:,}.",
            "",
            md_table(
                ...
                [
                    [
                        label,
                        pct(r["test"][pid]["quality"]),
                        f"${r['test'][pid]['mean_cost_usd']:.8f}",
                        pct(r["macro"][pid]["quality"]),
                        f"${r['macro'][pid]['mean_cost_usd']:.8f}",
                    ]
                    for pid, label in [
                        ("best_single", "Best single"),
                        ("deployable_router", "Learned router"),
            ...
            "",
            f"Noninferiority supported (lower 95% quality-difference CI ≥ −0.5 pp): **{'yes' if r['bootstrap']['noninferiority_supported'] else 'no'}**. "
            + (
                "Quality preservation within the stated margin is supported."
                if r["bootstrap"]["noninferiority_supported"]
                else "Do not call quality preserved."
            ),
            "",
            ...
            md_table(
                ...
                [
                    [
                        DATASETS[d],
                        x["deployable_router"]["n"],
                        pct(x["deployable_router"]["quality"]),
                        pct(x["best_single"]["quality"]),
                        f"{100 * (x['deployable_router']['quality'] - x['best_single']['quality']):+.3f}",
                        f"${x['deployable_router']['mean_cost_usd']:.7f}",
                        f"${x['best_single']['mean_cost_usd']:.7f}",
                        pct(x["always_cheapest"]["quality"]),
                        pct(x["oracle"]["quality"]),
                    ]
            ...
            md_table(
                ...
                [
                    [
                        NAMES.get(x["model_id"], x["model_id"]),
                        pct(x["prevalence"]),
                        num(x["roc_auc"]),
                        num(x["pr_auc"]),
                        num(x["log_loss"]),
            ...
            md_table(
                ...
                [
                    [x["policy_id"], pct(x["quality"]), f"${x['mean_cost_usd']:.8f}"]
                    for x in r["pareto_frontier"]
                ],
            ),
            "",
            md_table(
                ...
                [
                    [
                        f"{100 * x['epsilon']:.1f} pp",
                        x["feasible"],
                        x["policy_id"],
                        pct(x["cost_savings"]),
                    ]
                    for x in r["envelopes"]["savings_at_best_single_quality"]
                ],
            ),
            "",
            md_table(
                [
                    "Budget / train best cost",
                    "Budget USD",
                    "Descriptive gain pp",
                    "Validation choice",
                ...
                [
                    [
                        ...
                        f"${x['budget_usd']:.6f}",
                        "infeasible"
                        if x["descriptive_quality_gain"] is None
                        else f"{100 * x['descriptive_quality_gain']:+.3f}",
                        x["validation_selected_policy_id"],
                        x["frozen_test_budget_violation"],
                    ]
                    for x in r["envelopes"]["fixed_budget"]
                ],
            ),
            "",
            f"![{heading} frontier](figures/{r['figure_prefix']}_cost_quality_frontier.png)",
            "",
            f"![{heading} calibration](figures/{r['figure_prefix']}_calibration.png)",
            "",
        ]
            ...
            md_table(
                ...
                [
                    [
                        r["regime"] + " / " + r["kind"],
                        r["selected_C"],
                        r["selected_policy"]["policy_id"],
                        pct(r["test"]["deployable_router"]["quality"]),
                        f"${r['test']['deployable_router']['mean_cost_usd']:.8f}",
                        pct(r["comparison"]["cost_savings"]),
                        f"{r['comparison']['quality_delta'] * 100:+.3f}",
                        ", ".join(
                            f"{x * 100:+.3f}" for x in r["bootstrap"]["ci_95"]["quality_delta"]
                        ),
                        r["bootstrap"]["noninferiority_supported"],
                    ]
                    for r in [
                        results[k]
                        for k in (
                            "standard_tfidf",
                            "standard_handcrafted",
                            "ood_tfidf",
                            "ood_handcrafted",
                        )
                        if k in results
    ...
    lines += [
        ...
        md_table(
            [
                ...
                "Training seconds",
                "Replay seconds",
                "Test prediction ms/prompt",
                "Test routing ms/prompt",
                "Vocabulary",
            ],
            [
                [
                    k,
                    f"{r['timing']['training_seconds']:.2f}",
                    f"{r['timing']['determinism_replay_seconds']:.2f}",
                    f"{r['timing']['test_prediction_ms_per_prompt']:.4f}",
                    f"{r['timing']['test_routing_ms_per_prompt']:.4f}",
                    r["vocabulary_size"],
                ]
