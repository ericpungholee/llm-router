...
def calibration(y, p):
    ...
    for b in range(10):
        mask = bins == b
        if mask.any():
            rows.append(dict(bin=b, left=b / 10, right=(b + 1) / 10, count=int(mask.sum()),
                             mean_probability=float(p[mask].mean()), observed_success=float(y[mask].mean())))
    reliability = pd.DataFrame(rows)
    ece = sum(r["count"] * abs(r["mean_probability"] - r["observed_success"]) for r in rows) / len(y)
    return dict(brier=float(np.mean((p - y) ** 2)), log_loss=clipped_log_loss(y, p), ece=float(ece)), reliability
...
def binary_metrics(y, p):
    ...
    return dict(prevalence=float(y.mean()), roc_auc=float(roc_auc_score(y, p)) if both_classes else None,
                ...
                accuracy_at_0_5=float(np.mean((p >= 0.5) == y)), **cal), rel
...
def outcome_summary(part, selected):
    ...
    for dataset in ["__micro__"] + sorted(set(datasets)):
        ...
        records.append(dict(dataset=dataset, n=int(mask.sum()), quality=float(y[mask].mean()),
                            mean_cost_usd=float(costs[mask].mean()), median_cost_usd=float(np.median(costs[mask])), coverage=1.0))
    macro = pd.DataFrame(records[1:])[["quality", "mean_cost_usd"]].mean().to_dict()
    records.append(dict(dataset="__macro__", n=len(rows), median_cost_usd=None, coverage=1.0, **macro))
...
def evaluate_policies(part, prediction, costs, selected, best):
    ...
    policies += [best, Policy("always_cheapest", "static", model_id=MODEL_IDS[cheapest(costs)]),
    ...
    actions["hindsight_cheapest_cost"] = np.lexsort((np.broadcast_to(np.asarray(MODEL_IDS), c.shape), c), axis=1)[:, 0]
    ...
    for policy_id, action in actions.items():
        ...
        identity = dict(prompt_id=part.prompts.index, policy_id=policy_id,
        ...
        evaluation.append(pd.DataFrame(dict(**identity, dataset=part.prompts.dataset.to_numpy(),
                                            leakage_group=part.prompts.leakage_group.to_numpy(),
                                            success=success, cost_usd=realized)))
    return pd.DataFrame(summaries), pd.concat(decisions, ignore_index=True), pd.concat(evaluation, ignore_index=True), actions


def nondominated(table):
    c, q = table.mean_cost_usd.to_numpy(), table.quality.to_numpy()
    return np.array([not np.any((c <= c[i]) & (q >= q[i]) & ((c < c[i]) | (q > q[i]))) for i in range(len(c))])
...
def envelopes(summary, best, costs, budgets):
    ...
    for epsilon in (0.005, 0.0):
        ...
        winner = None if feasible.empty else feasible.sort_values(["mean_cost_usd", "quality"], ascending=[True, False]).iloc[0]
        savings.append(dict(epsilon=epsilon, feasible=winner is not None,
                            policy_id=None if winner is None else winner.name,
                            cost_savings=None if winner is None else float(1 - winner.mean_cost_usd / baseline.mean_cost_usd)))
    ...
    for frozen in budgets:
        ...
        budget_results.append(dict(**frozen, descriptive_feasible=feasible,
                                   descriptive_quality_gain=float(routed.quality.max() - fixed.quality.max()) if feasible else None,
                                   frozen_test_quality=None if pid is None else float(micro.loc[pid, "quality"]),
                                   frozen_test_cost_usd=None if pid is None else float(micro.loc[pid, "mean_cost_usd"]),
                                   frozen_test_budget_violation=None if pid is None else bool(micro.loc[pid, "mean_cost_usd"] > b)))
    return dict(descriptive_only=True, savings_at_best_single_quality=savings, fixed_budget=budget_results)
...
def selection_tables(part, prediction, action):
    ...
    for j, model_id in enumerate(MODEL_IDS):
        mask = action == j
        records.append(dict(model_id=model_id, count=int(mask.sum()), fraction=float(mask.mean()),
    ...
    cross = pd.crosstab(part.prompts.dataset.to_numpy(), np.asarray(MODEL_IDS)[action]).reindex(columns=MODEL_IDS, fill_value=0)
