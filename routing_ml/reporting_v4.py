...
DOMAIN_NAMES = dict(aime="AIME", gpqa="GPQA", livecodebench="LiveCodeBench", livemathbench="LiveMathBench",
...
def figures(output, reports, report):
    ...
    for regime in ("standard", "ood"):
        ...
        ax.scatter(static.mean_cost_usd, static.quality * 100, marker="s", color=".65", s=28, label="Eight static models")
        ax.scatter(grid.mean_cost_usd, grid.quality * 100, color="#2166ac", s=34, label="Fixed margin grid")
        for pid, label, color, marker in (("fixed_gpt5", "Fixed GPT-5", "black", "D"),
                                          ("primary", "Frozen primary", "#b2182b", "*"),
                                          ("oracle", "Hindsight oracle", "#238b45", "P")):
            row = table.loc[pid]
            ax.scatter(row.mean_cost_usd, row.quality * 100, color=color, marker=marker, s=135 if pid == "primary" else 60, label=label, zorder=5)
        ax.set_xscale("log")
        ax.set(xlabel="Mean recorded cost (USD/prompt; log scale)", ylabel="Success (%)", title=regime.upper() + " test cost / quality")
        ax.legend(fontsize=8, loc="best")
        d = r["per_dataset"]
        y = np.arange(len(d))
        quality = np.array([x["quality_delta"] * 100 for x in d])
        intervals = np.array([x["bootstrap"]["ci_95"]["quality_delta"] for x in d]) * 100
        axes[1].errorbar(quality, y, xerr=np.maximum(0, np.vstack([quality - intervals[:, 0], intervals[:, 1] - quality])),
                         fmt="o", color="#2166ac", capsize=3)
        axes[1].set_yticks(y, [f"{DOMAIN_NAMES[x['dataset']]} (n={x['n']})" for x in d])
        axes[1].invert_yaxis()
        axes[1].axvline(0, color=".7", lw=1)
        axes[1].axvline(-.5, color="#b2182b", lw=1, ls="--")
        axes[1].set(xlabel="Primary − fixed GPT-5 quality (pp)", title="Paired descriptive 95% intervals")
    ...
    for ax, regime in zip(axes, ("standard", "ood")):
        ...
        ax.set(xlim=(0, 1), ylim=(0, 1), xlabel="Predicted selected-model success", ylabel="Observed success", title=regime.upper())
...
def generate_report(output, reports):
    ...
    for regime in ("standard", "ood"):
        ...
        actions = pd.read_parquet(output / regime / 'source_holdout_actions.parquet').set_index('prompt_id')[selected]
        ...
        reference = evidence[evidence.policy_id == 'fixed_gpt5'].set_index('prompt_id').loc[routed.index]
        records = []
        for domain in ['__micro__'] + sorted(routed.dataset.unique()):
            rows = routed if domain == '__micro__' else routed[routed.dataset == domain]
            records.append(dict(dataset=domain, n=len(rows), gpt5_fraction=float(actions.loc[rows.index].eq(6).mean()),
                                cost_savings=float(1-rows.cost_usd.mean()/reference.loc[rows.index].cost_usd.mean())))
    ...
    for regime in ("standard", "ood"):
        ...
        lines += [f"**{regime.upper()}:** frozen primary `{r['frozen_selection']['primary']['candidate_id']}` achieves "
    ...
    lines += ["These are exploratory results on already-inspected benchmark outcomes. Fixing the reference prevents "
    ...
    lines += ["The standard result clears the previous observed code-quality loss by selecting a more conservative "
    ...
    for regime in ("standard", "ood"):
        ...
        worst = bounds.loc[bounds.groupby("policy_id").joint_lower.idxmin()].sort_values("policy_id")
        lines += [f"## {regime.upper()} test", "",
                  ...
                  md_table(["Policy", "Quality", "Cost USD/prompt", "Dataset macro quality"],
                           [[pid, pct(r['micro'][pid]['quality']), f"${r['micro'][pid]['mean_cost_usd']:.8f}", pct(r['macro'][pid]['quality'])]
                            for pid in ("primary", "fixed_gpt5", "validation_best_single", "always_cheapest", "oracle")]), "",
                  ...
                  md_table(["Declared criterion", "Pass"], [[k, str(v)] for k, v in p['conditions'].items()]), "",
                  ...
                  "### Frozen controls", "",
                  md_table(["Policy", "Validation choice", "Quality", "Cost USD", "Savings", "Δ pp", "95% Δ CI pp"],
                           [[name, frozen['primary']['candidate_id'] if name == 'primary' else frozen['controls']['ordinary_only' if name == 'ordinary_only_control' else 'fixed_margin'],
                             pct(row['quality']), f"${row['cost_usd']:.8f}", pct(row['cost_savings']), pp(row['quality_delta']), ci(row['bootstrap']['ci_95']['quality_delta'])]
                  ...
                  md_table(["Candidate", "Validation quality", "Cost USD", "Worst joint lower pp", "Joint feasible", "Ordinary-only feasible"],
                           [[row.policy_id, pct(row.quality), f"${row.mean_cost_usd:.8f}", pp(row.worst_joint_lower), str(row.feasible), str(row.ordinary_only_feasible)]
                            for row in candidates.itertuples()]), "",
                  md_table(["Candidate", "Most limiting block", "Metric/domain", "Observed Δ pp", "Joint lower pp"],
                           [[row.policy_id, row.block, row.metric, pp(row.quality_delta), pp(row.joint_lower)] for row in worst.itertuples()]), "",
                  ...
                  md_table(["Source holdout", "N", "Routed to GPT-5", "Cost savings"],
                           [[DOMAIN_NAMES.get(row['dataset'], 'All source holdouts'), row['n'], pct(row['gpt5_fraction']), pct(row['cost_savings'])]
                            for row in behavior[regime]]), "",
                  "### Dataset outcomes", "",
                  md_table(["Dataset", "N", "Primary quality", "Δ vs GPT-5 pp", "95% Δ CI pp", "Savings"],
                           [[DOMAIN_NAMES[d['dataset']], d['n'], pct(d['quality']), pp(d['quality_delta']), ci(d['bootstrap']['ci_95']['quality_delta']), pct(d['cost_savings'])]
                            for d in r['per_dataset']]), "",
                  md_table(["Excluded from micro evaluation", "N remaining", "Δ pp", "Savings"],
                           [[', '.join(DOMAIN_NAMES[d] for d in row['excluded']), row['n'], pp(row['quality_delta']), pct(row['cost_savings'])]
                            for row in r['composition_checks']]), "",
                  "### Selected-model behavior", "",
                  md_table(["Model", "N", "Share", "Mean prediction", "Actual success", "Mean cost USD"],
                           [[row['model_id'], row['count'], pct(row['fraction']), '—' if row['mean_predicted_success'] is None else pct(row['mean_predicted_success']),
                             '—' if row['actual_success'] is None else pct(row['actual_success']), '—' if row['mean_cost_usd'] is None else f"${row['mean_cost_usd']:.8f}"]
                            for row in r['distribution']]), "",
                  "### Predictor diagnostics", "",
                  md_table(["Predictor", "Prevalence", "ROC AUC", "PR AUC", "Log loss", "Brier", "ECE", "Accuracy"],
                           [[row['model_id']] + ['—' if row[k] is None else f"{row[k]:.4f}" for k in
                            ('prevalence','roc_auc','pr_auc','log_loss','brier','ece','accuracy_at_0_5')] for row in r['classifier_metrics']]), ""]
        recovered = 'undefined' if p['oracle_gap_recovered'] is None else pct(p['oracle_gap_recovered'])
        lines += [f"Primary is on the descriptive test Pareto frontier among this fixed grid and static references: "
    ...
    lines += ["## Matched code comparison", "",
              ...
              "Run into an unused directory to preserve completed experiments:", "", "```bash",
              ...
              "Route new prompt text through the frozen primary locally:", "", "```bash",
    ...
    if tests.exists():
        ...
        lines += [f"Full offline suite: **{t['tests_run']} tests, {t['failures']} failures, {t['errors']} errors, "
    ...
    (reports / "router_v4_results.json").write_text(json.dumps(result, indent=2, sort_keys=True, allow_nan=False) + "\n")
