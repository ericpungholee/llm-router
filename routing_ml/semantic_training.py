...
from routing_ml.training import C_GRID, LOGREG_PARAMS, MODEL_IDS, SEED, clipped_log_loss, digest, fit_predictors, probabilities
...
def train_semantic(train, embeddings, family="embedding", progress=print):
    ...
    with threadpool_limits(limits=1):
        for fold, (fit_idx, score_idx) in enumerate(folds.split(raw, train.prompts.dataset, groups)):
            ...
            preprocessing_audit.append(dict(fold=fold, mean=scaler.mean_.tolist(), scale=scaler.scale_.tolist(),
                                            training_prompt_ids_hash=digest(train.prompts.index[fit_idx].tolist())))
            assignments.extend(dict(prompt_id=train.prompts.index[i], fold=fold,
                                    leakage_group=groups[i], dataset=train.prompts.dataset.iloc[i]) for i in score_idx)
            for c in C_GRID:
    ...
    metadata = dict(seed=SEED, selected_C=c, family=family, candidate_model_order=list(MODEL_IDS),
                    training_prompt_ids_hash=digest(train.prompts.index.tolist()), training_prompt_count=len(train.prompts),
                    training_dataset_counts=train.prompts.dataset.value_counts().sort_index().to_dict(),
                    cv_mean_per_model_log_loss=scores, cv_stratification="dataset", cv_preprocessing=preprocessing_audit,
                    feature_columns=columns, logistic_parameters=LOGREG_PARAMS,
                    scaler_mean=scaler.mean_.tolist(), scaler_scale=scaler.scale_.tolist(),
                    final_constant_fallbacks=fallbacks, final_n_iter=[int(m.n_iter_.max()) if hasattr(m, "n_iter_") else 0 for m in models],
                    fit_split="train", refit_train_validation=False, calibration="raw probabilities")
    return SemanticModel(train.regime, family, c, scaler, models, metadata), cv, pd.DataFrame(assignments).sort_values("prompt_id")
