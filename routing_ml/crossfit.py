...
def partition_assignments(prompts):
    ...
    for fold, (remaining, test) in enumerate(outer.split(ids, prompts.dataset, groups)):
        ...
        fit, validation = next(
            inner.split(ids[remaining], prompts.dataset.iloc[remaining], groups[remaining])
        )
        ...
        for name, indices in parts.items():
            records.extend(
                dict(
                    ...
                    fold=fold,
                    role=name,
                    dataset=prompts.dataset.iloc[i],
...
def validate_assignments(table, prompts):
    ...
    for _, fold in table.groupby("fold"):
        if set(fold.prompt_id) != set(prompts.index) or set(fold.role) != {
            "train",
            "validation",
            "test",
        }:
...
def fit_fixed_router(train):
    ...
    metadata = dict(
        seed=SEED,
        selected_C=FIXED_C,
        C_selection="frozen from v2; no v3 tuning",
        ...
        vocabulary_size=len(prep.vocabulary_),
        final_constant_fallbacks=fallbacks,
        fit_split="crossfit_train",
...
def freeze_references(train, validation):
    ...
    return dict(
        regime=train.regime,
        selected_C=FIXED_C,
        margin=MARGINS[train.regime],
        best_single=best.model_id,
        mean_training_cost_usd=dict(zip(MODEL_IDS, costs)),
        domain_static_models=domains,
        always_cheapest=MODEL_IDS[cheapest(costs)],
...
def frozen_actions(prediction, datasets, frozen):
    ...
    if (
        ...
        or ((prediction < 0) | (prediction > 1)).any().any()
    ):
    ...
    n = len(prediction)
    result = dict(
        router=comparative_route(prediction, costs, best, frozen["margin"]),
        best_single=np.full(n, best),
        always_cheapest=np.full(n, MODEL_IDS.index(frozen["always_cheapest"])),
        privileged_domain_static=np.array(
            [
                MODEL_IDS.index(frozen["domain_static_models"].get(d, frozen["best_single"]))
                for d in datasets
            ]
        ),
    )
