...
def grid_actions(prediction, costs):
    ...
    return pd.DataFrame(np.column_stack(actions).astype(np.int8), index=prediction.index, columns=POLICY_IDS)
...
def source_partitions(train):
    ...
    return {d: (p.index[p.dataset != d], p.index[p.dataset == d]) for d in sorted(p.dataset.unique())}
...
class Evidence:
    ...
    def __post_init__(self):
        ...
        for a in (self.quality_difference, self.costs, self.quality):
            if np.asarray(a).shape != (len(self.prompts), len(POLICY_IDS)) or not np.isfinite(a).all():
                raise ValueError("Evidence matrix alignment failure")
        if not np.isin(self.quality, [0, 1]).all() or not np.isin(self.quality_difference, [-1, 0, 1]).all():
...
def evidence_from_actions(part, actions, name):
    ...
    selected_y = y[rows, a]
    return Evidence(name, part.regime, part.prompts.copy(), selected_y - y[:, REFERENCE_INDEX, None],
                    c[rows, a], selected_y)
...
def _statistics(prompts, values, indices=None):
    ...
    return ["__micro__", "__macro__"] + names, np.vstack([selected.mean(axis=0), domain.mean(axis=0), domain])
...
def joint_bounds(blocks, replicates=2000, seed=SEED):
    ...
    if set(ordinary.prompts.index) & set(source.prompts.index) or set(ordinary.prompts.leakage_group) & set(source.prompts.leakage_group):
    ...
    for name in ("ordinary", "source_holdout"):
        ...
        for i, metric in enumerate(labels):
            rows.extend(dict(block=name, metric=metric, policy_id=pid, quality_delta=float(observed[i, j]),
                             standard_error=float(se[i, j])) for j, pid in enumerate(POLICY_IDS))
    max_t = np.maximum(maxima["ordinary"], maxima["source_holdout"])
    critical = float(np.quantile(max_t, .95))
    ordinary_critical = float(np.quantile(maxima["ordinary"], .95))
    table = pd.DataFrame(rows)
    table["joint_lower"] = table.quality_delta - critical * table.standard_error
    table["ordinary_only_lower"] = np.where(table.block.eq("ordinary"), table.quality_delta - ordinary_critical * table.standard_error, np.nan)
    samples = pd.DataFrame(dict(replicate=np.arange(replicates), joint_max_t=max_t,
                                ordinary_max_t=maxima["ordinary"], source_holdout_max_t=maxima["source_holdout"]))
    metadata = dict(replicates=replicates, seed=seed, reference=REFERENCE, critical_value=critical,
                    ordinary_only_critical_value=ordinary_critical, comparison_count=len(table),
                    method="paired stratified whole-group joint max-t lower bounds", confidence=.95,
                    conditional_on_fitted_models=True, independent_confirmation=False,
...
def select_policy(blocks, replicates=2000, seed=SEED):
    ...
    for j, pid in enumerate(POLICY_IDS):
        ...
        rows.append(dict(policy_id=pid, quality=float(ordinary.quality[:, j].mean()),
                         mean_cost_usd=float(ordinary.costs[:, j].mean()), worst_joint_lower=lower,
                         worst_ordinary_lower=ordinary_lower, feasible=lower >= -.005,
                         ordinary_only_feasible=ordinary_lower >= -.005))
    ...
    def choose(column):
        return candidates[candidates[column]].sort_values(["mean_cost_usd", "quality", "policy_id"], ascending=[True, False, True]).iloc[0].policy_id
    selected = choose("feasible")
    control = choose("ordinary_only_feasible")
    metadata.update(selected_policy=selected, ordinary_only_control=control, feasible_count=int(candidates.feasible.sum()))
