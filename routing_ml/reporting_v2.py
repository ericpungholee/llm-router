...
def plot_results(output, reports, regime):
    ...
    for family in FAMILIES:
        ...
        ax.scatter(rows.mean_cost_usd * 1000, rows.quality * 100, color=colors[family], s=14, alpha=0.35,
                   label=family.replace("_", " + "))
    static = table[table.policy_id.str.startswith("static_")]
    ax.scatter(static.mean_cost_usd * 1000, static.quality * 100, marker="s", color="black", s=24, label="Static models")
    for row in static.itertuples():
        name = row.policy_id.removeprefix("static_")
        ax.annotate(NAMES[name], (row.mean_cost_usd * 1000, row.quality * 100), xytext=(4, 6), textcoords="offset points", fontsize=7)
    for pid, label, marker, color in [("primary", "Frozen primary", "*", "#009E73"), ("oracle", "Oracle (hindsight)", "D", "#CC79A7")]:
        row = table[table.policy_id == pid].iloc[0]
        ax.scatter(row.mean_cost_usd * 1000, row.quality * 100, marker=marker, color=color, edgecolors="black",
                   linewidths=0.6, s=180 if pid == "primary" else 60, label=label, zorder=6)
    frontier = table[table.pareto_deployable].drop_duplicates(["mean_cost_usd", "quality"]).sort_values("mean_cost_usd")
    ax.plot(frontier.mean_cost_usd * 1000, frontier.quality * 100, color="#555555", alpha=0.5, linewidth=1)
    ...
    fig, axes = plt.subplots(2, 4, figsize=(11, 5.5), sharex=True, sharey=True, layout="constrained")
    for model_id, ax in zip(MODEL_IDS, axes.flat):
        ...
        ax.scatter(rows.mean_probability, rows.observed_success, s=12 + 3*np.sqrt(rows["count"]), color="#0072B2")
        ax.set(title=NAMES[model_id], xlim=(0, 1), ylim=(0, 1), xticks=[0, 0.5, 1], yticks=[0, 0.5, 1])
        ax.grid(alpha=0.15)
    fig.supxlabel("Mean predicted success (10 fixed bins)")
    fig.supylabel("Observed success")
    fig.suptitle(f"Router v2 — {regime.upper() if regime == 'ood' else 'standard'} embedding-head reliability")
    for ext in ("png", "pdf"):
...
def generate_report(root_output, reports):
    ...
    runs = {r: json.loads((root_output / r / "results.json").read_text()) for r in ("standard", "ood") if (root_output / r / "results.json").exists()}
    if not runs:
        return
    standard = runs["standard"]
    primary = standard["comparison"]["primary"]
    verdict = ("The frozen v2 standard router meets all four preregistered exploratory criteria: at least 10% savings, micro and macro noninferiority bounds, and the per-dataset point guard." if primary["promising_exploratory"] else
    ...
    lines = ["# Router v2: local semantic features and conservative routing", "", verdict, "",
    ...
    for regime, r in runs.items():
        ...
        b = r["baselines"]
        lines += [f"## {'Standard' if regime == 'standard' else 'Independent OOD'} test", "",
                  ...
                  md_table(["Policy", "Success", "Mean cost USD"], [["Frozen primary", pct(p["quality"]), f"${p['cost_usd']:.8f}"]] +
                           [[name, pct(b[pid]["quality"]), f"${b[pid]['mean_cost_usd']:.8f}"] for pid, name in [("best_single", "Best single"), ("always_cheapest", "Always cheapest"), ("oracle", "Oracle (hindsight)")]]), "",
                  ...
                  md_table(["Preregistered condition", "Pass"], [[k.replace("_", " "), v] for k, v in p["conditions"].items()]), "",
                  ...
                  md_table(["Selected policy on validation", "Value"], [[key, r["validation_primary"][key]] for key in ["quality", "mean_cost_usd", "micro_delta", "macro_delta", "micro_lower", "macro_lower", "worst_dataset_delta"]]), "",
                  "### Dataset outcomes", "",
                  md_table(["Dataset", "N", "Primary", "Best single", "Δ pp", "Primary cost", "Best cost"],
                           [[DATASETS[d], x["primary"]["n"], pct(x["primary"]["quality"]), pct(x["best_single"]["quality"]),
                             f"{100*(x['primary']['quality']-x['best_single']['quality']):+.3f}", f"${x['primary']['mean_cost_usd']:.8f}", f"${x['best_single']['mean_cost_usd']:.8f}"] for d, x in r["datasets"].items()]), "",
                  "### Representation controls", "", "Each control uses the original v1 minimum-cost validation selector over its 29 ungated policies. These isolate feature changes; none can replace the primary using test performance.", "",
                  md_table(["Features", "C", "Control policy", "Quality", "Cost USD", "Savings", "Δ pp", "95% Δ CI pp"],
                           [[f, r["families"][f]["selected_C"], r["controls"][f]["policy_id"], pct(r["comparison"]["control_"+f]["quality"]),
                             f"${r['comparison']['control_'+f]['cost_usd']:.8f}", pct(r["comparison"]["control_"+f]["cost_savings"]),
                             f"{100*r['comparison']['control_'+f]['quality_delta']:+.3f}",
                             ", ".join(f"{100*x:+.3f}" for x in r["comparison"]["control_"+f]["bootstrap"]["ci_95"]["quality_delta"])] for f in FAMILIES]), "",
                  "### Classifier diagnostics", "", "Macro below means an equal average over eight predictors, not dataset macro. PR AUC is average precision; accuracy uses p ≥ 0.5. Full per-model/train/validation/test and reliability tables are saved separately.", "",
                  md_table(["Features", "ROC AUC", "PR AUC", "Log loss", "Brier", "ECE", "Accuracy"],
                           [[f] + [num(next(x for x in r["test_classifier_metrics"] if x["family"] == f and x["model_id"] == "__macro__")[metric]) for metric in ["roc_auc", "pr_auc", "log_loss", "brier", "ece", "accuracy_at_0_5"]] for f in FAMILIES]), "",
                  "### Selected-model behavior", "",
                  md_table(["Model", "Count", "Routed", "Mean prediction", "Actual success", "Cost USD"],
    ...
    if "ood" in runs:
        ...
        comparison = dict(standard_code_n=len(a), standard_code_quality=float(a.success.mean()),
                          ood_code_n=len(b), ood_code_quality=float(b.success.mean()),
                          ood_minus_standard_pp=float(100*(b.success.mean()-a.success.mean())),
                          common_n=len(common), ood_common_quality=float(common.success.mean()),
        ...
        comparison = None
    lines += ["## What changed and what remains unresolved", "",
    ...
    lines += [f"The frozen encoder processed all 8,706 prompts locally in {elapsed:.2f} measured inference seconds ({elapsed*1000/8706:.2f} ms per prompt on average, excluding model loading). Neither selected primary needs that encoder at runtime. These are batch CPU timings, not provider latency or an invented dollar charge.", "",
              ...
              "## Local inference", "", "The saved primary is usable on new text through a label-free, offline command. It returns all eight predicted success probabilities, the selected benchmark model ID, and the training-derived estimated cost; it does not call that model.", "",
              "```bash", ".venv/bin/python experiments/route_local.py --prompt 'What is the capital of France?'", ".venv/bin/python experiments/route_local.py --artifact-dir artifacts/router_v2/ood --input requests.jsonl", "```", "",
              ...
              "```bash", ".venv/bin/python -m pip install -r experiments/requirements-router-v2.txt", ".venv/bin/python -m routing_ml.embeddings --download", ".venv/bin/python -m routing_ml.embeddings", ".venv/bin/python experiments/embedding_logreg_router.py", ".venv/bin/python tests/run_offline_suite.py", "```", "",
              ...
              "```bash", "router_replay_dir=$(mktemp -d /tmp/router-v2-replay.XXXXXX)",
    ...
    if domain_path.exists():
        ...
        lines += ["## Post-hoc diagnostic: privileged domain-static reference", "",
                  ...
                  md_table(["Standard reference", "Success", "Mean cost USD"], [[x["policy_id"], pct(x["quality"]), f"${x['mean_cost_usd']:.8f}"] for x in rows]), "",
    ...
    if test_path.exists():
        ...
        lines += [f"Full offline suite: **{tests['tests_run']} tests; {tests['failures']} failures, {tests['errors']} errors, {tests['skipped']} skipped; {tests['unexpected_network_attempts']} network attempts**. [Test summary](router_v2_tests.json).", ""]
    (reports / "router_v2_results.md").write_text("\n".join(lines))
    (reports / "router_v2_results.json").write_text(json.dumps(dict(runs=runs, code_comparison=comparison), indent=2, sort_keys=True) + "\n")
