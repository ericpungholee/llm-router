...
from experiments.tfidf_logreg_router import COUNTS, clean, model_signature, sha256, verify_source, write_json
...
from routing_ml.fixed_reference import (MARGINS, POLICY_IDS, REFERENCE, REFERENCE_INDEX,
                                        evidence_from_actions, evidence_from_predictions,
                                        grid_actions, select_policy, source_partitions)
from routing_ml.metrics import classifier_diagnostics, oracle_actions, pareto_table, selection_tables
...
def start_campaign(output, data):
    ...
    paths += [ROOT / "experiments/tfidf_logreg_router.py", ROOT / "experiments/embedding_logreg_router.py",
              ROOT / "experiments/requirements-router-v1.txt", ROOT / "experiments/route_local.py"]
    source_hashes = {}
    for p in paths:
    ...
    prior = {str(p.relative_to(ROOT)): sha256(p) for version in (1, 2, 3)
             for p in sorted((ROOT / "reports").glob(f"router_v{version}*")) if p.is_file()}
    lock = dict(seed=SEED, fixed_reference=REFERENCE, source_processed_data_hashes=hashes,
                source_hashes=source_hashes, prior_report_hashes=prior,
...
def subset(part, ids, role):
    return CrossfitPart(part.regime, role, part.prompts.loc[ids].copy(), part.y.loc[ids].copy(), part.costs.loc[ids].copy())


def fit(train, output, excluded_domain=None, replay=False):
    start = time.perf_counter()
    model = fit_fixed_router(train)
    model.metadata.update(C_selection="C=1 fixed before v4; no tuning", fit_split="canonical_train" if excluded_domain is None else "source_domain_train",
    ...
    if replay:
        ...
        other.metadata.update(C_selection="C=1 fixed before v4; no tuning", fit_split="canonical_train" if excluded_domain is None else "source_domain_train",
                              excluded_source_domain=excluded_domain)
    ...
    pd.testing.assert_frame_equal(prediction, joblib.load(output / "models.joblib").predict(train), check_exact=True)
    costs, normalized = estimate_costs(train)
    metadata = dict(**model.metadata, model_signature=signature, training_seconds=time.perf_counter() - start,
                    training_replay_exact=bool(replay), mean_training_cost_usd=dict(zip(MODEL_IDS, costs)),
...
def source_evidence(train, output):
    ...
    for domain, (fit_ids, score_ids) in source_partitions(train).items():
        ...
        for role, part in (("train", fitting), ("validation", heldout)):
            assignments.append(part.prompts[["dataset", "leakage_group"]].reset_index().assign(excluded_domain=domain, role=role))
        print(f"{train.regime}: held out source domain {domain}; fitted {len(fit_ids)}, validated {len(score_ids)}", flush=True)
    ...
    pd.concat(assignments, ignore_index=True).to_parquet(output / "source_assignments.parquet", index=False)
...
def save_evidence(evidence, output):
    ...
    for j, pid in enumerate(POLICY_IDS):
        records.append(pd.DataFrame(dict(prompt_id=evidence.prompts.index, policy_id=pid,
                                        dataset=evidence.prompts.dataset.to_numpy(), leakage_group=evidence.prompts.leakage_group.to_numpy(),
                                        success=evidence.quality[:, j], quality_delta=evidence.quality_difference[:, j], cost_usd=evidence.costs[:, j])))
    pd.concat(records, ignore_index=True).to_parquet(output / f"{evidence.name}_validation_evaluation.parquet", index=False)


def runtime_candidate(policy_id):
    if policy_id == "fixed_gpt5":
        return dict(candidate_id=policy_id, family="static", kind="static", value=0, gate_quantile=0)
    return dict(candidate_id=policy_id, family="tfidf", kind="advantage", value=MARGINS[POLICY_IDS.index(policy_id)], gate_quantile=0)
...
def evaluate(test, prediction, actions, costs, frozen, output):
    ...
    records = []
    for j, pid in enumerate(policy_ids):
        records.append(pd.DataFrame(dict(prompt_id=test.prompts.index, policy_id=pid,
                                        selected_model_id=np.asarray(MODEL_IDS)[matrix[:, j]], dataset=test.prompts.dataset.to_numpy(),
                                        leakage_group=test.prompts.leakage_group.to_numpy(), success=y[:, j], cost_usd=c[:, j])))
    pd.concat(records, ignore_index=True).to_parquet(output / "test_evaluation.parquet", index=False)
    ...
    for name in ("primary", "ordinary_only_control", "fixed_margin_control"):
        ...
        comparison["bootstrap"].update(independent_confirmation=False, interpretation="exploratory; conditional on frozen fit and selection")
    ...
    comparisons["primary"]["all_dataset_interval_margins_pass"] = all(d["bootstrap"]["ci_95"]["quality_delta"][0] >= -.005 for d in per_domain)
    ...
    for excluded in (("simpleqa",), ("simpleqa", "mmlupro")):
        ...
        composition.append(dict(excluded=list(excluded), n=int(mask.sum()), quality_delta=float((y[mask, r] - y[mask, b]).mean()),
                                cost_savings=float(1 - c[mask, r].mean() / c[mask, b].mean())))
    source_metadata = {p.parent.name: json.loads(p.read_text()) for p in sorted((output / "source_holdout").glob("*/metadata.json"))}
    return dict(regime=test.regime, selected_C=1, fixed_reference=REFERENCE, frozen_selection=frozen,
                comparisons=comparisons, micro=micro.to_dict(orient="index"), macro=macro.to_dict(orient="index"),
                per_dataset=per_domain, distribution=distribution.to_dict(orient="records"),
                dataset_model_counts=counts.to_dict(orient="records"), classifier_metrics=diagnostics.to_dict(orient="records"),
                composition_checks=composition, primary_on_pareto=bool(frontier.set_index("policy_id").loc["primary", "pareto_deployable"]),
                source_fit_metadata=source_metadata, independent_confirmation=False)
...
def run_regime(data, prompts, output, regime):
    ...
    validation_prediction.reset_index().to_parquet(path / "validation_predictions.parquet", index=False)
    ...
    selected, control, candidates, bounds, samples, selection_metadata = select_policy(dict(ordinary=ordinary, source_holdout=source))
    ...
    frozen = dict(primary=runtime_candidate(selected), reference=Policy("fixed_gpt5", "static", model_id=REFERENCE).to_dict(),
                  validation_best_single=selected_best.to_dict(), controls=dict(ordinary_only=control, fixed_margin=fixed_control),
                  mean_training_cost_usd=dict(zip(MODEL_IDS, costs)), normalized_training_cost=dict(zip(MODEL_IDS, costs / costs.max())),
                  candidate_model_order=list(MODEL_IDS), seed=SEED, selected_C=1,
                  selection_metadata=selection_metadata, test_used_for_selection=False,
                  training_prompt_ids_hash=digest(train.prompts.index.tolist()), validation_prompt_ids_hash=digest(validation.prompts.index.tolist()),
    ...
    print(f"{regime}: frozen primary={selected}; ordinary-only={control}; joint feasible={selection_metadata['feasible_count']}/6", flush=True)
    ...
    write_json(path / "artifact_hashes.json", {str(p.relative_to(path)): sha256(p) for p in sorted(path.rglob("*")) if p.is_file() and p.name != "artifact_hashes.json"})
...
def matched_code(output):
    ...
    for regime in ("standard", "ood"):
        ...
        frames.append(e[(e.policy_id == "primary") & (e.dataset == "livecodebench")].set_index("prompt_id").sort_index())
    standard, ood = frames
    ...
    return dict(shared_n=len(shared), standard_quality=float(b.success.mean()), ood_shared_quality=float(a.success.mean()),
                ood_full_quality=float(ood.success.mean()), quality_delta_shared_ood_minus_standard=float((a.success - b.success).mean()),
                bootstrap=bootstrap, independent_confirmation=False)
...
def main():
    ...
    with ExitStack() as stack:
        ...
        result = dict(regimes=results, provenance=lock, matched_code=matched_code(args.output_dir),
                      prior_results_and_processed_data_unchanged=True, independent_confirmation=False)
