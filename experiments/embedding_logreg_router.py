...
from experiments.tfidf_logreg_router import checked_read, clean, model_signature, sha256, verify_source, write_json
from routing_ml.bootstrap import paired_bootstrap
from routing_ml.conservative import Candidate, NoveltyGate, candidate_actions, candidates, conservative_select
from routing_ml.embeddings import ENCODER_CONFIG, load_cache
from routing_ml.metrics import classifier_diagnostics, nondominated, oracle_actions, selection_tables
...
def campaign_lock(output, data):
    ...
    paths = sorted((ROOT / "routing_ml").glob("*.py")) + [
        ROOT / "experiments/embedding_logreg_router.py", ROOT / "experiments/tfidf_logreg_router.py",
        ROOT / "experiments/requirements-router-v2.txt", ROOT / "experiments/requirements-router-v1.txt",
        ROOT / "reports/router_v2_experiment_spec.md"]
    manifest = dict(source_processed_data_hashes=sources,
                    ...
                    encoder_config=ENCODER_CONFIG, existing_test_status="previously inspected; exploratory only")
    output.mkdir(parents=True, exist_ok=True)
    lock = output / "campaign_lock.json"
    if lock.exists():
        previous = json.loads(lock.read_text())
        if previous != manifest:
            raise ValueError("Campaign protocol/source changed; preserve this run and use a new output directory")
    else:
...
def summarize_actions(part, actions, policy_ids):
    ...
    for dataset in ["__micro__"] + sorted(set(datasets)):
        ...
        summaries.append(pd.DataFrame(dict(policy_id=policy_ids, dataset=dataset, n=int(mask.sum()),
                                            quality=y[mask].mean(axis=0), mean_cost_usd=c[mask].mean(axis=0),
                                            median_cost_usd=np.median(c[mask], axis=0))))
    macro = pd.concat(summaries[1:]).groupby("policy_id", sort=False)[["quality", "mean_cost_usd"]].mean().reindex(policy_ids)
...
def test_comparison(part, action, best_action, oracle_action):
    ...
    samples, bootstrap = paired_bootstrap(part.prompts, y[rows, action], y[rows, best_action], c[rows, action], c[rows, best_action])
    summary, _, _ = summarize_actions(part, np.column_stack([action, best_action, oracle_action]), ["router", "best_single", "oracle"])
    micro = summary[summary.dataset == "__micro__"].set_index("policy_id")
    macro = summary[summary.dataset == "__macro__"].set_index("policy_id")
    by_dataset = summary[~summary.dataset.str.startswith("__")].pivot(index="dataset", columns="policy_id", values="quality")
    delta = float(micro.loc["router", "quality"] - micro.loc["best_single", "quality"])
    savings = float(1 - micro.loc["router", "mean_cost_usd"] / micro.loc["best_single", "mean_cost_usd"])
    oracle_gap = float(micro.loc["oracle", "quality"] - micro.loc["best_single", "quality"])
    conditions = dict(savings_at_least_10_percent=savings >= 0.1,
    ...
    return samples, dict(quality=float(micro.loc["router", "quality"]), cost_usd=float(micro.loc["router", "mean_cost_usd"]),
                         quality_delta=delta, cost_savings=savings, macro_quality=float(macro.loc["router", "quality"]),
                         macro_quality_delta=float(macro.loc["router", "quality"] - macro.loc["best_single", "quality"]),
                         ...
                         bootstrap=bootstrap, conditions=conditions, promising_exploratory=all(conditions.values()))
...
def evaluate_frozen(test, predictions, costs, gate, similarities, chosen, best, controls, output):
    ...
    catalog = candidates()
    main_actions = candidate_actions(predictions, costs, baseline_index, similarities, gate, catalog)
    policy_ids = [c.candidate_id for c in catalog] + ["best_single"]
    main_actions = np.column_stack([main_actions, np.full(len(test.prompts), baseline_index, dtype=np.int8)])
    primary_action = main_actions[:, policy_ids.index(chosen.candidate_id)]
    refs = {"static_" + m: np.full(len(test.prompts), j, dtype=np.int8) for j, m in enumerate(MODEL_IDS)}
    ...
    pd.DataFrame(actions, index=test.prompts.index, columns=policy_ids).reset_index().to_parquet(output / "test_actions.parquet", index=False)
    ...
    families = {c.candidate_id: c.family for c in catalog}
    comparison = {}
    ...
    decisions = []
    for name in ["primary", "best_single", "always_cheapest", "oracle"] + ["control_" + f for f in FAMILIES]:
        j = policy_ids.index(name)
        decisions.append(pd.DataFrame(dict(prompt_id=test.prompts.index, policy_id=name,
                                            ...
                                            success=y[:, j], cost_usd=c[:, j])))
    evaluation = pd.concat(decisions, ignore_index=True)
    evaluation.to_parquet(output / "test_evaluation.parquet", index=False)
    evaluation.drop(columns=["success", "cost_usd", "dataset", "leakage_group"]).to_parquet(output / "test_routing_decisions.parquet", index=False)
    datasets = {d: summary[summary.dataset == d].set_index("policy_id")[["quality", "mean_cost_usd", "n"]].loc[
        ["primary", "best_single", "always_cheapest", "oracle"]].to_dict(orient="index") for d in sorted(test.prompts.dataset.unique())}
    base = summary[summary.dataset == "__micro__"].set_index("policy_id")
    return dict(comparison=comparison, baselines=base.loc[["best_single", "always_cheapest", "oracle"]].to_dict(orient="index"),
                datasets=datasets, selection_distribution=distribution.to_dict(orient="records"),
                dataset_selection_counts=cross.to_dict(orient="records"),
                primary_prediction_reference_family=primary_family,
                gate_rejection_fraction=float(np.mean(similarities < gate.cutoffs[chosen.gate_quantile])) if chosen.gate_quantile else 0.0,
                primary_on_pareto_frontier=bool(frontier.loc[frontier.policy_id == "primary", "pareto_deployable"].iloc[0]),
                frontier=frontier[frontier.pareto_deployable].to_dict(orient="records")), primary_action


def run_regime(data, root_output, reports, regime, embeddings, cache_metadata, lock):
    output = root_output / regime
    if (output / "results.json").exists():
        raise ValueError("Completed regime already exists; use --report-only or a fresh output directory")
    output.mkdir(parents=True, exist_ok=True)
    ...
    for family in FAMILIES:
        ...
        if family in {"tfidf", "handcrafted"}:
            model, cv, folds = train_router(train, family, lambda message: print(message, flush=True))
        else:
        ...
        pd.testing.assert_frame_equal(prediction, predict_family(joblib.load(directory / "models.joblib"), train, embeddings), check_exact=True)
        prediction.reset_index().to_parquet(directory / "train_predictions.parquet", index=False)
        train_predictions[family] = prediction
        family_metadata[family] = dict(selected_C=model.selected_c, model_signature=model_signature(model),
                                       training_seconds=time.perf_counter() - start, metadata=model.metadata)
    ...
    pd.testing.assert_frame_equal(replay_cv, pd.read_parquet(output / "embedding/cv_scores.parquet"), check_exact=True)
    ...
    pd.DataFrame(dict(prompt_id=train.prompts.index, similarity=train_similarity)).to_parquet(output / "train_novelty.parquet", index=False)
    ...
    print(f"{regime}: computing simultaneous validation bounds before loading test outcomes", flush=True)
    chosen, best, table, samples, selection_metadata, val_actions = conservative_select(validation, validation_predictions, costs, validation_similarity, gate)
    ...
    pd.DataFrame(val_actions, index=validation.prompts.index, columns=table.candidate_id).reset_index().to_parquet(output / "validation_actions.parquet", index=False)
    pd.DataFrame(dict(prompt_id=validation.prompts.index, similarity=validation_similarity)).to_parquet(output / "validation_novelty.parquet", index=False)
    frozen = dict(primary=chosen.to_dict(), best_single=best.to_dict(), controls={f: p.to_dict() for f, p in controls.items()},
                  mean_training_cost_usd=dict(zip(MODEL_IDS, costs)), normalized_training_cost=(costs/costs.max()).tolist(),
                  novelty_cutoffs=gate.cutoffs, selection=selection_metadata,
                  model_signatures={f: m["model_signature"] for f, m in family_metadata.items()},
                  protocol_sha256=lock["protocol_sha256"], frozen_before_test_outcomes_loaded=True)
    write_json(output / "frozen_selection.json", frozen)
    frozen_hash = sha256(output / "frozen_selection.json")
    print(f"{regime}: frozen {chosen.candidate_id}; feasible={selection_metadata['feasible_count']}; max-t={selection_metadata['critical_value']:.3f}", flush=True)
    ...
    for family, model in models.items():
        ...
        for part, prediction in [(train, train_predictions[family]), (validation, validation_predictions[family]), (test, p)]:
    ...
    pd.DataFrame(dict(prompt_id=test.prompts.index, similarity=test_similarity)).to_parquet(output / "test_novelty.parquet", index=False)
    results, primary_action = evaluate_frozen(test, test_predictions, costs, gate, test_similarity, chosen, best, controls, output)
    ...
    results.update(regime=regime, primary=chosen.to_dict(), best_single=best.to_dict(),
                   controls={f: p.to_dict() for f, p in controls.items()}, selection_metadata=selection_metadata,
                   validation_primary=table[table.candidate_id == chosen.candidate_id].iloc[0].to_dict(),
                   families={f: {k: m[k] for k in ("selected_C", "model_signature", "training_seconds")} for f, m in family_metadata.items()},
                   ...
                   semantic_training_replay_exact=True, source_processed_data_hashes=lock["source_processed_data_hashes"],
                   frozen_selection_sha256=frozen_hash, protocol_sha256=lock["protocol_sha256"],
                   embedding_cache_metadata=cache_metadata, novelty_cutoffs=gate.cutoffs,
    ...
    if sha256(output / "frozen_selection.json") != frozen_hash or verify_source(data) != lock["source_processed_data_hashes"]:
        raise AssertionError("Frozen policy or data changed")
    write_json(output / "results.json", results)
    write_json(output / "artifact_hashes.json", {str(p.relative_to(output)): sha256(p) for p in sorted(output.rglob("*"))
                                                if p.is_file() and p.name != "artifact_hashes.json"})
    print(f"{regime}: primary results {json.dumps(clean(results['comparison']['primary']))}", flush=True)
...
def main():
    ...
    prompt_text = pd.read_parquet(args.data_dir / "prompts.parquet", columns=["prompt_id", "prompt"]).sort_values("prompt_id").reset_index(drop=True)
    ...
    with ExitStack() as stack:
        ...
        for regime in ("standard", "ood"):
            ...
            run_regime(args.data_dir, args.output_dir, args.reports_dir, regime, embeddings, cache_metadata, lock)
            generate_report(args.output_dir, args.reports_dir)
