...
def run_fold(data, output, prompts, assignments, regime, fold):
    ...
    ids = {
        name: fold_ids(assignments, fold, regime, name) for name in ("train", "validation", "test")
    }
    ...
    pd.testing.assert_frame_equal(
        train_prediction, joblib.load(path / "models.joblib").predict(train), check_exact=True
    )
    for name, part in (("train", train), ("validation", validation)):
        model.predict(part).reset_index().to_parquet(
            path / f"{name}_predictions.parquet", index=False
        )
    ...
    frozen.update(
        ...
        training_replay_exact=replay_exact,
        training_metadata=model.metadata,
        decisions_saved_before_evaluation_labels=True,
    )
    ...
    decisions = pd.DataFrame(
        {p: np.asarray(MODEL_IDS)[a] for p, a in actions.items()}, index=prediction.index
    )
    ...
    pd.concat(evaluation, ignore_index=True).to_parquet(
        path / "test_evaluation.parquet", index=False
    )
    if sha256(path / "frozen_policy.json") != frozen_hash:
        raise AssertionError("Frozen reference changed during evaluation")
    print(
        f"{regime} fold {fold + 1}/5: fit {len(train.prompts)}, validation {len(validation.prompts)}, "
        f"held out {len(test.prompts)}, best={frozen['best_single']}; replay={replay_exact}",
        flush=True,
    )
...
def summarize_rows(evaluation):
    ...
    for pid, values in evaluation.groupby("policy_id", sort=True):
        domains = values.groupby("dataset").agg(
            quality=("success", "mean"), mean_cost_usd=("cost_usd", "mean"), n=("success", "size")
        )
        records.append(
            dict(
                policy_id=pid,
                dataset="__micro__",
                quality=float(values.success.mean()),
                mean_cost_usd=float(values.cost_usd.mean()),
                n=len(values),
            )
        )
        records.extend(
            dict(policy_id=pid, dataset=d, **row)
            for d, row in domains.to_dict(orient="index").items()
        )
        records.append(
            dict(
                policy_id=pid,
                dataset="__macro__",
                quality=float(domains.quality.mean()),
                mean_cost_usd=float(domains.mean_cost_usd.mean()),
                n=len(values),
            )
        )
...
def comparison(evaluation, reference, output, label):
    router = (
        evaluation[evaluation.policy_id == "router"].sort_values("prompt_id").set_index("prompt_id")
    )
    baseline = (
        evaluation[evaluation.policy_id == reference]
    ...
    samples, bootstrap = paired_bootstrap(
        router, router.success, baseline.success, router.cost_usd, baseline.cost_usd
    )
    samples.to_parquet(output / f"{label}_bootstrap_samples.parquet", index=False)
    bootstrap.update(
        interpretation="descriptive conditional intervals; overlapping fold fits and prior benchmark adaptation excluded",
        independent_confirmation=False,
    )
    return dict(
        quality_delta=float((router.success - baseline.success).mean()),
        cost_savings=float(1 - router.cost_usd.mean() / baseline.cost_usd.mean()),
        bootstrap=bootstrap,
    )
...
def summarize_regime(data, output, prompts, assignments, regime):
    ...
    evaluation = pd.concat(
        [pd.read_parquet(path / f"fold_{k}/test_evaluation.parquet") for k in range(5)],
        ignore_index=True,
    )
    evaluation = evaluation.sort_values(["policy_id", "prompt_id"]).reset_index(drop=True)
    prediction = (
        pd.concat([pd.read_parquet(path / f"fold_{k}/test_predictions.parquet") for k in range(5)])
        .set_index("prompt_id")
        .sort_index()
    )
    expected_ids = (
        prompts.index if regime == "standard" else prompts[prompts.dataset == "livecodebench"].index
    )
    ...
    for dataset in sorted(evaluation.dataset.unique()):
        ...
        per_dataset.append(
            dict(
                dataset=dataset,
                n=int(s.loc["router", "n"]),
                router=s.loc["router"].to_dict(),
                best_single=s.loc["best_single"].to_dict(),
                **dcomp,
            )
        )
    folds = []
    for k in range(5):
        ...
        folds.append(
            dict(
                ...
                vocabulary_hash=frozen["training_metadata"]["vocabulary_hash"],
                quality_delta=float(m.loc["router", "quality"] - m.loc["best_single", "quality"]),
                cost_savings=float(
                    1 - m.loc["router", "mean_cost_usd"] / m.loc["best_single", "mean_cost_usd"]
                ),
                router=m.loc["router"].to_dict(),
                baseline=m.loc["best_single"].to_dict(),
            )
        )
    test = read_part(data, prompts, prediction.index, regime, "test")
    selected = (
        evaluation[evaluation.policy_id == "router"].set_index("prompt_id").loc[prediction.index]
    )
    ...
    checks = dict(
        savings_at_least_10_percent=comp["cost_savings"] >= 0.1,
    ...
    result = dict(
        regime=regime,
        fixed_C=1.0,
        margin=MARGINS[regime],
        n=len(prediction),
        micro=micro.to_dict(orient="index"),
        macro=macro.to_dict(orient="index"),
        primary=comp,
        privileged_reference_comparison=domain,
        per_dataset=per_dataset,
        folds=folds,
        fold_checks=dict(
            quality_margin_passes=sum(f["quality_delta"] >= -0.005 for f in folds),
            savings_10_percent_passes=sum(f["cost_savings"] >= 0.1 for f in folds),
        ),
        descriptive_checks=checks,
        all_descriptive_checks_pass=all(checks.values()),
        ...
        composition_checks=excluded,
    )
    write_json(path / "results.json", result)
    print(
        f"{regime}: quality={micro.loc['router', 'quality']:.6f}, delta={comp['quality_delta']:.6f}, "
        f"savings={comp['cost_savings']:.6f}; descriptive checks={checks}",
        flush=True,
    )
    return clean(result)
...
def matched_code(output):
    ...
    for regime in ("standard", "ood"):
        ...
        frames.append(
            e[(e.policy_id == "router") & (e.dataset == "livecodebench")]
            .set_index("prompt_id")
            .sort_index()
        )
    standard, ood = frames
    ...
    samples, bootstrap = paired_bootstrap(
        ood, ood.success, standard.success, ood.cost_usd, standard.cost_usd
    )
    samples.to_parquet(output / "matched_code_bootstrap_samples.parquet", index=False)
    return dict(
        n=len(ood),
        standard_quality=float(standard.success.mean()),
        ood_quality=float(ood.success.mean()),
        ...
        bootstrap=bootstrap,
        interpretation="matched prompts; independently fitted systems with different training domains",
    )
...
def main():
    ...
    with ExitStack() as stack:
        ...
        for regime in ("standard", "ood"):
            ...
            results[regime] = summarize_regime(
                args.data_dir, args.output_dir, prompts, assignments, regime
            )
