...
def main():
    ...
    report = dict(
        status="post-hoc diagnostic; not preregistered primary or deployment candidate",
        privileged_input="dataset name",
        selection_split="validation",
        unseen_dataset_rule="validation-selected global best-single",
        regimes={},
    )
    for regime in ("standard", "ood"):
        ...
        domain = np.asarray(
            [MODEL_IDS.index(choices.get(d, fallback)) for d in test.prompts.dataset]
        )
        ...
        summary, y, c = summarize_actions(
            test, np.column_stack([primary, domain]), ["frozen_primary", "privileged_domain_static"]
        )
        ...
        report["regimes"][regime] = dict(
            selected_domain_models=choices,
            fallback=fallback,
            metrics=summary.to_dict(orient="records"),
            bootstrap=bootstrap,
            primary_minus_domain_quality=float(
                micro.loc["frozen_primary", "quality"]
                - micro.loc["privileged_domain_static", "quality"]
            ),
            primary_cost_savings_vs_domain=float(
                1
                - micro.loc["frozen_primary", "mean_cost_usd"]
                / micro.loc["privileged_domain_static", "mean_cost_usd"]
            ),
        )
