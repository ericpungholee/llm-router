...
from routing_ml.training import MODEL_IDS, digest, read_split, train_router

COUNTS = {
    "standard": {"train": 6094, "validation": 1307, "test": 1305},
...
def verify_source(directory):
    ...
    names = [
        ...
        "outcomes.parquet",
        "splits.parquet",
        "prompt_features.parquet",
        "static_model_summary.parquet",
    ]
    ...
    prompts = pd.read_parquet(
        directory / "prompts.parquet",
        columns=["prompt_id", "leakage_group", "standard_split", "ood_split"],
    )
    splits = pd.read_parquet(directory / "splits.parquet")
    pd.testing.assert_frame_equal(
        prompts.set_index("prompt_id").sort_index(),
        splits.set_index("prompt_id").sort_index()[prompts.columns[1:]],
    )
...
def run_one(directory, output, reports, regime, kind):
    ...
    pd.testing.assert_frame_equal(
        train_prediction, joblib.load(output / "models.joblib").predict(train), check_exact=True
    )
    ...
    frozen = dict(
        ...
        frozen_before_test_read=True,
        test_used_for_selection=False,
    )
    write_json(output / "frozen_policy.json", frozen)
    frozen_hash = sha256(output / "frozen_policy.json")
    progress(
        f"{regime}/{kind}: selected C={model.selected_c:g}, validation policy={selected.policy_id}; frozen before test load"
    )
    ...
    for part, prediction in [
        ...
        if part.name != "train":
            summary, decisions, evaluation, actions = evaluate_policies(
                part, prediction, costs, selected, best
            )
    ...
    samples, bootstrap = paired_bootstrap(
        test.prompts,
        selected_rows.success,
        best_rows.success,
        selected_rows.cost_usd,
        best_rows.cost_usd,
    )
    ...
    def aggregate(dataset):
        return (
            test_summary[test_summary.dataset == dataset]
            .drop(columns=["split", "dataset"])
            .set_index("policy_id")
    ...
    code_paths = sorted((ROOT / "routing_ml").glob("*.py")) + [
        Path(__file__),
    ...
    metadata = dict(
        **model.metadata,
        ...
        spec_sha256=sha256(ROOT / "reports/ml_experiment_spec.md"),
    ...
    result = dict(
        ...
        comparison=dict(
            ...
            macro_quality_delta=macro_delta,
            macro_cost_savings=macro_savings,
        ...
        oracle_conditional_cost_usd=float(
            oracle_rows.loc[oracle_rows.success == 1, "cost_usd"].mean()
        ),
        validation_best_quality=float(
            val_grid.loc[val_grid.policy_id == "best_single", "quality"].iloc[0]
        ),
        ...
        envelopes=envelopes(test_summary, best, costs, budgets),
        ...
        timing=dict(
            ...
            determinism_replay_seconds=replay_seconds,
    ...
    progress(
        f"{regime}/{kind}: test quality={micro['deployable_router']['quality']:.6f}, savings={savings:.6f}, delta={delta:.6f}, CI={bootstrap['ci_95']['quality_delta']}"
    )
    return clean(result)
...
def update_reports(output, reports):
    ...
    for regime in COUNTS:
        for kind in ("tfidf", "handcrafted"):
            path = (
                output / regime / ("handcrafted" if kind == "handcrafted" else "") / "results.json"
            )
            if path.exists():
    ...
    if "standard_tfidf" in results and "ood_tfidf" in results:
        ...
        results["ood_comparison"] = dict(
            standard_code_quality=standard["quality"],
            standard_code_n=standard["n"],
            ood_code_quality=ood["quality"],
            ood_code_n=ood["n"],
            standard_code_cost_usd=standard["mean_cost_usd"],
            ood_code_cost_usd=ood["mean_cost_usd"],
            ood_minus_standard_quality=ood["quality"] - standard["quality"],
            common_code_n=len(oe),
            ood_common_quality=float(oe.success.mean()),
        ...
        for kind in ("tfidf", "handcrafted"):
            ...
            for regime in COUNTS:
                ...
                if regime == "ood" or kind == "handcrafted":
                    ...
                    if (
                        not prerequisite.exists()
                        or not json.loads(prerequisite.read_text())[
                            "determinism_full_training_replay_verified"
                        ]
                    ):
