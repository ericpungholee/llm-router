...
This is archived infrastructure from the separate five-model paid pilot. The
learned-router experiments use eight frozen LLMRouterBench candidates instead.
Provider identifiers and prices below describe the recorded pilot configuration;
they are not a claim about current availability or pricing.

The offline ML workflow needs no API keys. Commands in the live sections can
spend money and are outside that workflow. The Python code grader is a local
subprocess runner with AST checks and timeouts, **not a security sandbox**.
Run untrusted generated programs only inside a separately isolated environment
without credentials, network access, or sensitive files.

Paths in shell examples assume the repository root.
...
At the time of the recorded analysis, both candidate paid treatments failed strict preflight because two
...
outcomes. `benchmarks/hard_pilot.json` identifies the completed 15-prompt pilot with
...
When inspecting this pilot, inspect completeness and failure categories first, then compare
